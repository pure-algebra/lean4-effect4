...
`LayerId` into a declared `LayerTable` is the model's stand-in and owes a `LAYER-FB-LAYER-IDENTITY`
refusal row of the `SCOPE-FB-KEY-IDENTITY` shape (`Effect4/Runtime/Scope.lean:314-320`). `forEach`
with `concurrency: layers.length` (`:1597-1598`) is modelled as "fork every layer's build, await
them all, merge" — the early interruption of the siblings on a first failure is not modelled and
...
def mergeForkAll (memoMap : MemoMapId) (parent : Nat) :
  ...
  | [], forked => Prim.onSuccess (act (ActionName.awaitAll forked)) Name.mergeContexts
  | layer :: rest, forked =>
...
theorem mergeAll_scopes (layer self : LayerId) (layers rest : List LayerId) (l : LayerId)
    ...
    layerBuildProgram table layer memoMap scope =
      ...
      mergeForkAll memoMap parent [] forked =
        Prim.onSuccess (act (ActionName.awaitAll forked)) Name.mergeContexts ∧
