...
The fiber machine (`workshop/Deep/Fibers.lean`) folded in spike S1's `Prim` constructors and
...
which is the same refusal `Effect4/Concurrency/FiberFamily.lean:68-71` already records, so
...
(`workshop/Deep/Fibers.lean:433-434`) and this reads one back, and the two are inverse. -/
...
(`workshop/Deep/Fibers.lean:701-709`). -/
...
first (P1), `caught` routes the caught failure, `close` appends the closing reasons, and
`fin` is an `onExit` finalizer name whose value arm is never demanded. -/
    ...
    contA := fun name value =>
      ...
      | RegionName.regionCont region point =>
        ...
        | Effect4.Exit.success _ =>
          ...
          | some row =>
            compileFork alphabet flow opOf (point.fuel - 1) row.continue_ [value] point.tape
          | none => Effect4.Prim.suspend point
...
and the async `Prim` constructors (`workshop/Deep/Fibers.lean:52-54`). -/
...
(`workshop/Deep/Fibers.lean:295-296`, `:896-898`) — the arm this spike's first pass asked
...
(`workshop/Deep/Fibers.lean:254-297`). -/
...
(`workshop/Deep/Fibers.lean:616-634`) builds its child by exactly this call. -/
...
fiber ids, tokens, primitives, annotations and booleans (`workshop/Deep/Fibers.lean:358-376`),
...
(`workshop/Deep/Fibers.lean:1168-1174`) is rc.112's `runForkWith`: the root is evaluated
...
(`workshop/Deep/Fibers.lean:731`), `drive` halts on it (`:1050-1051`) and `replayEval`
