...
/-- Law 4 (merge associativity), pointwise. The data-level equality also holds (positions of
existing keys are kept and new keys are appended in operand order on both sides) but is not
proved here; `Equiv` is what a program observes. -/
