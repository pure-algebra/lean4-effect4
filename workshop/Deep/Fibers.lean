...
def countdownPark (interp : RunInterp ν σ β ε δ ι α χ St) (m : RunMachine ν σ β ε δ ι α χ St)
  ...
  if live.isEmpty then
    ...
    let f := f.park ⟨token, none, live.length, exited, resumeWith⟩
...
def iteration (interp : RunInterp ν σ β ε δ ι α χ St) (m : RunMachine ν σ β ε δ ι α χ St)
  ...
  if !yielding && !f.preventYield && verdict then
    ...
    let f := f.park ⟨token, none, 0, [], Resume.void⟩
      ...
      | some (Except.ok (ParkKind.join target mode)) =>                -- :5291, :5304
        ...
        | some t =>
          ...
          | none =>
            ...
            let f := f.park ⟨token, some target, 0, [], Resume.void⟩
            ⟨m.emit [RunEvent.parkedOn f.id token], f, yielding, Outcome.parked, []⟩
      | none =>
        match f.frame.current with
...
where
  ...
  finalizerOr (interp : RunInterp ν σ β ε δ ι α χ St) (m : RunMachine ν σ β ε δ ι α χ St)
    ...
    | (Prim.onExit body fin flag) :: rest =>
      ...
      | some program =>
        let (fiber, _) := (Prim.onExit body fin flag).ensure { f.frame with stack := rest }
        let fiber := { fiber with
          current := Prim.onSuccessAndFailure program (interp.restoreName exit) (interp.mergeName exit) }
        ⟨m.emit [RunEvent.frame f.id (FrameEvent.popped (Prim.onExit body fin flag)),
            RunEvent.finalizerProgram f.id fin exit],
      ...
      | some p =>
        ...
        if p.remaining <= 1 then
          let w := { w with pending := w.pending.map fun q =>
            if q.token = token then { q with remaining := 0, collected := collected } else q }
          (m.update w,
            acc.2 ++ [Cmd.resume waiter token (countdownPark.resumePrim interp p.resumeWith collected)])
        else
          let w := { w with pending := w.pending.map fun q =>
            if q.token = token then { q with remaining := q.remaining - 1, collected := collected }
            else q }
          (m.update w, acc.2)
