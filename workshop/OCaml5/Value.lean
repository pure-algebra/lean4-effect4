...
`workshop/OCaml5/values/`, run on four hosts by `values/run-values.sh` (ocamlopt, ocamlc +
...
Every row below was printed by a program under `workshop/OCaml5/values/`, compiled and run by
