...
`workshop/OCaml5/promise/w1..w8-*.mjs` with their `.out` files, run under Node v22.23.2; the
OCaml+js_of_ocaml witness is `workshop/OCaml5/promise/p6_await.ml` with
...
First-order and closed, per the O3 value profile (`workshop/OCaml5/Value.lean`). The only
