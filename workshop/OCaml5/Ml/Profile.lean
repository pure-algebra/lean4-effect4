...
The law names are the interface to seat W4, which is proving them under `workshop/OCaml5/Lib/`.
...
under `workshop/OCaml5/Lib/`; `site` is where, and `none` means not yet. The name never changes,
...
structure Law where
  ...
  statement : String
  /-- `workshop/OCaml5/Lib/<file>.lean`, and the theorem, once W4 has it. -/
...
def laws : List Law :=
  -- The `Map.*` names are seat W4's own, from `workshop/OCaml5/Lib/Map.lean`'s "Named
...
def libOption : LibModule where
  ...
  doc := "the option type"
  types := ["t"]
  carrier := some "workshop/OCaml5/Value.lean (Value.none, Value.some)"
  values :=
...
def libInt : LibModule where
  ...
  doc := "63-bit integers"
  types := ["t"]
  carrier := some "workshop/OCaml5/Value.lean (Value.int)"
  values :=
...
def libEffect : LibModule where
  ...
  types := ["t"]
  carrier := some "workshop/OCaml5/Effect.lean (Term.perform, Machine.step)"
  values :=
...
def libEffectDeep : LibModule where
  ...
  carrier :=
    some "workshop/OCaml5/Effect.lean (Stdlib.deepMatchWith, deepContinue, deepDiscontinue)"
  values :=
...
def libEffectShallow : LibModule where
  ...
  carrier := some "workshop/OCaml5/Effect.lean (Stdlib.shallowFiber, shallowContinueWith)"
  values :=
    [{ name := "fiber", ty := .anon },
...
-- Every law of the three `Effect` modules has a site in `workshop/OCaml5/Effect.lean`, and no
...
-- (`workshop/OCaml5/Effect.lean`) and W4's thirteen `Map.*` (`workshop/OCaml5/Lib/Map.lean`),
