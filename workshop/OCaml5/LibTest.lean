...
def dsp : Effect4.Deep.Dispatcher Nat Nat Nat Nat Nat Nat Nat :=
  ((Effect4.Deep.Dispatcher.empty.enqueue 1 (Task.start ⟨7⟩)).enqueue 0
    (Task.start ⟨8⟩)).enqueue 1 (Task.start ⟨9⟩)

open Effect4.Deep in
/-- The same three `enqueue`s on `Buckets`. -/
def bsp : Buckets (Effect4.Deep.Task Nat Nat Nat Nat Nat Nat Nat) :=
...
def probeStruct : Ml.StructDesc :=
  { leanName := "Probe"
    site := "workshop/OCaml5/LibTest.lean"
    subst := []
    fields :=
