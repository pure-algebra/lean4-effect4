#!/usr/bin/env python3
"""Generate `e4d_armmap.ml` from the avatar's sources and from `Effect4/Deep/Fibers.lean`.
...
Five tables are emitted:
  ...
  symbols      label -> the `Effect4/Deep/Fibers.lean` declarations its doc names by name.
...
Citations resolve against every `Effect4/Deep/*.lean` module, not only `Fibers.lean`: seat
W1's port moved the store arms into `deep_stores.ml`, whose arms cite `Stores.lean`. A bare
`:N` resolves against the *home* module of the avatar file it was written in
(`deep_stores.ml` -> `Stores.lean`), which is the avatar's own convention.
...
def repo_relative(path):
    ...
    for marker in ("/Effect4/", "/workshop/"):
...
def main():
    ...
    print("(* GENERATED by workshop/OCaml5/server/tools/gen_armmap.py. Do not edit. *)")
    print("(* Sources: %s, and Effect4/Deep/Fibers.lean *)"
    ...
    print("]")
    print()
    print("(* handler label -> the Lean declarations of Effect4/Deep/*.lean its doc names. *)")
