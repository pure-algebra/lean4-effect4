...
# under node.
#
# The daemon links the avatar read-only. Seat W1 owns `workshop/OCaml5/avatar/*`; this script
...
# `server_runtime.js` (the pattern of `workshop/OCaml5/promise/build-ocaml.sh`). A dune build
...
#                          resolved against Effect4/Deep/Fibers.lean
...
stage_and_build() {
  ...
  avatar_build=$(git -C "$repo" show "${W2_AVATAR_REV}:workshop/OCaml5/avatar/build-avatar.sh")
  else
  ...
  for module in $avatar_sources; do
    git -C "$repo" show "${W2_AVATAR_REV}:workshop/OCaml5/avatar/$module" > "$build/$module"
  done
  git -C "$repo" show "${W2_AVATAR_REV}:workshop/OCaml5/avatar/corpus/programs.txt" \
    > "$build/programs.txt"
...
print("(* workshop/OCaml5/avatar/corpus/known-divergences.tsv *)")
...
PY
  ...
  # Every Deep module, comma separated: the store arms cite `Stores.lean` and the layer arms
  # `Layer.lean` since seat W1's port, and a citation has to resolve wherever it points.
  deep_leans=$(printf '%s,' "$repo"/Effect4/Deep/*.lean | sed 's/,$//')
