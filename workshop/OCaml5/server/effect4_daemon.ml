...
let header_lines ~(label : string) ~(tape : string) ~(rules : string) : string list =
  [ "format\teffect4-trace-golden-v1";
    Printf.sprintf "generator\tworkshop/OCaml5/server/build-server.sh\tsha256=%s"
      E4d_pins.generator_sha;
...
let header_json ~label ~tape ~rules : E4d_json.t =
  Obj
    [ ("format", Str "effect4-trace-v1");
      ...
      ("generator", Str "workshop/OCaml5/server/build-server.sh");
      ("generatorSha256", Str E4d_pins.generator_sha);
      ("regenerate", Str "./workshop/OCaml5/server/build-server.sh");
      ( "inputs",
        List
          (List.map
...
let scan (r : run) (vary : int -> run) (target_rows : int) =
  let rec go step acc =
    if step > scan_limit then List.rev acc
      ...
      if List.length partial.rows >= target_rows && partial.events = r.events then List.rev acc
      else go (step + 1) acc
  in
  Array.of_list (go 0 [])
...
let json_of_arm (arm : string) : E4d_json.t =
  ...
  Obj
    [ ("arm", Str arm);
      ...
      ("avatar", Str ("workshop/OCaml5/avatar :: " ^ arm));
      ("doc", Str doc);
...
let reachable (m : run_machine) : E4d_json.t =
  let waiting_reason (f : run_fiber) (token : int) : E4d_json.t =
    ...
    | Some { waiting_on = Some target; remaining; _ }, _, _ ->
      Obj [ ("on", Str "fiber"); ("fiber", Int target); ("remaining", Int remaining) ]
    | Some { waiting_on = None; remaining; _ }, _, _ when remaining > 0 ->
      Obj [ ("on", Str "countdown"); ("remaining", Int remaining) ]
...
let version_payload () =
  [ ("protocol", Str protocol);
    ...
    ("pin", Obj [ ("effects", Str E4d_pins.pin); ("source", Str "generated/traces/masks.tsv") ]);
    ("generatorSha256", Str E4d_pins.generator_sha);
    ( "inputs",
      List
        (List.map
...
let dispatch (s : session) (request : E4d_json.t) : session * (string * E4d_json.t) list =
    ...
    [ ( "masks",
        List
          (List.map
             ...
             (Lazy.force builtin_masks)) );
      ("source", Str "generated/traces/masks.tsv") ]
  | "programs" ->
    keep
    [ ( "fixtures",
        List
  ...
  | "diff" ->
    ...
    [ ("verdicts", List (List.map E4d_wire.json_of_verdict verdicts));
      ...
      ( "rule",
        Str
          "workshop/OCaml5/avatar/compare.py, itself a transcription of \
