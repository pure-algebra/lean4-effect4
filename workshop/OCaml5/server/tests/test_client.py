...
native, js_of_ocaml under node) -- and asserts:
  ...
  rows-vs-avatar      the daemon's `run` rows are byte-identical to the avatar's own
                      committed face (`workshop/OCaml5/avatar/out/*.ocaml.tsv`), which is
...
REPO = os.path.abspath(os.path.join(SERVER, "..", "..", ".."))
AVATAR_OUT = os.path.join(REPO, "workshop", "OCaml5", "avatar", "out")
TRACES = os.path.join(REPO, "generated", "traces")
DEFAULT_BUILD = os.environ.get(
    "W2_BUILD",
    "/private/tmp/claude-501/-Users-pooks-Dev-lean4-effect4/"
    "d87ba830-2e63-4750-815f-2679b36f870a/scratchpad/w2") + "/build"
...
def main():
    ...
    ocaml_bin = os.environ.get("OCAML5_BIN", "/Users/pooks/.opam/default/bin")
    hosts = {
        ...
        "jsoo": (["node", os.path.join(args.build, "effect4d.js")], True),
    }
    ...
    node_argv = ["node", os.path.join(args.build, "effect4d.js")]
    if os.path.exists(node_argv[1]):
