...
(`workshop/OCaml5/avatar/`, the OCaml 5 transcription of `Effect4/Deep/*.lean`) and answers
questions about the runs, over newline-delimited JSON.
...
```
./build-server.sh                      # all three hosts, into $W2_BUILD/build
tests/run-tests.sh                     # build, then every test on every host
W2_AVATAR_REV=HEAD ./build-server.sh   # build the avatar half from a git revision
```
...
would be stated over the same carrier once `workshop/OCaml5/Lib/` has one.
...
its end) and `stack-overflow`. A malformed request is never fatal: the session continues.
...
generator	workshop/OCaml5/server/build-server.sh	sha256=…
regenerate	./workshop/OCaml5/server/build-server.sh
input	workshop/OCaml5/avatar/deep_fibers.ml	sha256=…      (one row per input, per-file digest)
…
...
`face` is `ocaml` because the rows *are* the avatar's: the daemon links the same modules
`avatar/build-avatar.sh` compiles and changes nothing about them. The `pin` is read at build
...
the `Effect4/Deep/*.lean` modules the arm citations resolve against.
...
tokens the avatar writes — each resolved against every `Effect4/Deep/*.lean` module as
...
8. **Citation resolution is partial, and shows drift.** Of the 32 citation tokens the avatar's
   handlers carry, 16 resolve `exact`, 12 `byLine` (reported unverified) and 4 `unresolved`.
   The `byLine` ones have drifted: the avatar cites `interruptRecord` at `:550` where
   `Effect4/Deep/Fibers.lean` declares it at 558, `spawn` at `:622` where it is at 631,
   `exitFiber` at `:992` where it is at 1031 and `drive` at `:1025` where it is at 1064 — the
   avatar was written against `workshop/Deep/Fibers.lean` before the promotion. This is why
   `explain` reports `leanDeclarations` by name as well.
...
10. **The `fiber` family has no committed golden in this checkout.** `generated/traces/`
    ...
    `workshop/OCaml5/avatar/out/*.ocaml.tsv` instead.
...
```
build-server.sh          all three hosts, five generated modules
...
tests/run-tests.sh       build and run everything
