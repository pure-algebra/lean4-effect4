...
let witnesses : witness list =
  [
    (* W1 -- fork and join *)
    theorem "w1_deferred_join_parent" "Witnesses.lean:287" [ ("w1DeferredJoin", w1_deferred_join) ]
      (fun d -> expect (exit_of (d "w1DeferredJoin") 0 = Some (Esuccess (Vnat 42))) "the parent does not exit with the joined child's success");
    theorem "w1_deferred_join_child" "Witnesses.lean:291" [ ("w1DeferredJoin", w1_deferred_join) ]
      (fun d ->
    ...
    theorem "w1_deferred_start_is_a_task" "Witnesses.lean:297" [ ("w1DeferredJoin[evaluate 0]", w1_deferred_join_no_fire) ]
      (fun d ->
    ...
    theorem "w1_immediate_join" "Witnesses.lean:307" [ ("w1ImmediateJoin", w1_immediate_join) ]
      (fun d ->
    ...
    theorem "w1_await_is_a_value" "Witnesses.lean:313" [ ("w1AwaitFailing", w1_await_failing) ]
      (fun d ->
    ...
    theorem "w3_empty_until_interrupted" "Witnesses.lean:389" [ ("w3EmptyInterrupted", w3_empty_interrupted) ]
    ...
    theorem "w3_immediate_success_stops_launch" "Witnesses.lean:397" [ ("w3StopsLaunch", w3_stops_launch) ]
      (fun d ->
    ...
    theorem "w3_failure_allows_next_launch" "Witnesses.lean:408" [ ("w3NextLaunch", w3_next_launch) ]
      (fun d ->
    ...
    theorem "w3_all_failures_retain_order" "Witnesses.lean:415" [ ("w3AllFail", w3_all_fail) ]
    ...
    theorem "w4_sibling_resumes" "Witnesses.lean:473" [ ("w4Sibling", w4_sibling) ]
      (fun d ->
    ...
    theorem "w4_completion_resumes_on_the_spot" "Witnesses.lean:482" [ ("w4SiblingThenMore", w4_sibling_then_more); ("w4Sibling", w4_sibling) ]
      (fun d ->
    ...
    theorem "w4_complete_twice_answers_false" "Witnesses.lean:489" [ ("w4CompleteTwice", w4_complete_twice) ]
      (fun d -> expect (exit_of (d "w4CompleteTwice") 0 = Some (Esuccess (Vbool false))) "the second completion did not answer false");
    theorem "w4_interrupt_reaches_the_waiter" "Witnesses.lean:494" [ ("w4InterruptWaiter", w4_interrupt_waiter) ]
      (fun d ->
    ...
    theorem "w5_middleware_interrupts_children" "Witnesses.lean:534" [ ("w5WithMiddleware", w5_with_middleware) ]
      (fun d ->
    ...
    theorem "w5_fork_latches_the_middleware" "Witnesses.lean:544" [ ("w5ForkLatches", w5_fork_latches); ("w5WithMiddleware", w5_with_middleware) ]
      (fun d ->
        ...
        all [ expect s.m.middleware_installed "the fork did not latch the middleware (R2-6)";
              ...
              expect (interrupt_rows s = [ (Some 0, 1) ]) "the interrupt rows differ";
              expect (exit_of s 0 = Some (Esuccess Vunit)) "the parent's exit differs" ]);
    theorem "w5_daemon_child_survives_parent_exit" "Witnesses.lean:554" [ ("w5Daemon", w5_daemon) ]
      (fun d ->
        let s = d "w5Daemon" in
        all [ expect (not s.m.middleware_installed) "a daemon fork latched the middleware";
              ...
              expect (exit_of s 0 = Some (Esuccess Vunit)) "the parent's exit differs" ]);
    theorem "w5_await_all_children_on_failure" "Witnesses.lean:580" [ ("w5AwaitAllChildrenFails", w5_await_all_children_fails); ("w5AwaitAllChildrenFailsFired", w5_await_all_children_fails_fired) ]
      (fun d ->
    ...
    theorem "w5_await_all_children_awaits_only_new" "Witnesses.lean:592" [ ("w5AwaitAllChildren", w5_await_all_children) ]
      (fun d ->
    ...
    (* W6 -- scope linkage *)
    theorem "w6_link_then_close" "Witnesses.lean:648" [ ("w6LinkThenClose", w6_link_then_close) ]
      (fun d ->
    ...
    theorem "w6_close_interrupt_carries_closer_annotations" "Witnesses.lean:661" [ ("w6LinkThenClose", w6_link_then_close); ("w2", w2) ]
      (fun d ->
    ...
    theorem "w6_closed_scope_interrupts_now" "Witnesses.lean:677" [ ("w6ClosedScope", w6_closed_scope) ]
      (fun d ->
    ...
    theorem "w6_runIn_closed_scope_uses_no_caller_annotations" "Witnesses.lean:687" [ ("w6ClosedRunIn", w6_closed_run_in) ]
      (fun d ->
    ...
    theorem "w6_child_exit_drops_key" "Witnesses.lean:697" [ ("w6DropsKey", w6_drops_key) ]
      (fun d ->
    ...
    theorem "w6_deferred_child_is_linked" "Witnesses.lean:715" [ ("w6DeferredLinked", w6_deferred_linked); ("w6DeferredLinkedFired", w6_deferred_linked_fired) ]
      (fun d ->
    ...
    theorem "w6_self_interruptor_skipped" "Witnesses.lean:724" [ ("w6SelfInterruptorSkipped", w6_self_interruptor_skipped) ]
      (fun d ->
    ...
    theorem "w6_sequential_captures_and_merges" "Witnesses.lean:755" [ ("w6Sequential", w6_sequential) ]
      (fun d ->
    ...
    theorem "w6_parallel_forks_and_merges" "Witnesses.lean:763" [ ("w6Parallel", w6_parallel) ]
      (fun d ->
    ...
    theorem "w10_program_masks_its_own_fiber" "Witnesses.lean:800" [ ("w10MaskedPending", w10_masked_pending); ("w10Masked", w10_masked) ]
      (fun d ->
    ...
    theorem "w10_into_completes_on_failure" "Witnesses.lean:815" [ ("w10Into", w10_into) ]
      (fun d ->
        let s = d "w10Into" in
    ...
    theorem "w11_cancel_splices_the_waiter" "Witnesses.lean:848" [ ("w11Parked", w11_parked); ("w11Cancelled", w11_cancelled) ]
      (fun d ->
    ...
    theorem "w12_awaitAll_answers_the_exits" "Witnesses.lean:870" [ ("w12AwaitAll", w12_await_all) ]
      (fun d ->
    ...
    theorem "w12_awaitAll_input_order" "Witnesses.lean:903" [ ("w12InputOrder", w12_input_order); ("w12InputOrderParked", w12_input_order_parked) ]
      (fun d ->
    ...
    theorem "w13_unknown_scope_is_stuck" "Witnesses.lean:929" [ ("w13UnknownScope", w13_unknown_scope) ]
      (fun d ->
    ...
    theorem "w13_unknown_link_is_stuck" "Witnesses.lean:937" [ ("w13UnknownLink", w13_unknown_link) ]
      (fun d ->
    ...
    (* W7 -- the ref.* quirks *)
    theorem "w7_set_answers_the_cell" "Witnesses.lean:969" [ ("w7Set", w7_set) ]
      (fun d ->
        let s = d "w7Set" in
        expect (exit_of s 0 = Some (Esuccess (Vhandle 0)) && s.refs = [ Vnat 5 ]) "`set` did not answer the cell and write the value");
    theorem "w7_update_answers_void" "Witnesses.lean:975" [ ("w7Update", w7_update) ]
      (fun d ->
        let s = d "w7Update" in
        expect (exit_of s 0 = Some (Esuccess Vunit) && s.refs = [ Vnat 2 ]) "`update` did not answer void with the function applied once");
    theorem "w7_modify_some_writes_back_the_read" "Witnesses.lean:981" [ ("w7ModifySome", w7_modify_some) ]
      (fun d ->
    ...
    theorem "w7_update_some_and_get_rereads" "Witnesses.lean:988" [ ("w7UpdateSomeAndGet", w7_update_some_and_get); ("w7GetAndUpdateSome", w7_get_and_update_some) ]
      (fun d ->
    ...
    theorem "w8_sync_exit_async_fiber_error" "Witnesses.lean:1015" []
    ...
    theorem "w8_sync_exit_value" "Witnesses.lean:1019" []
      (fun _ -> expect (w8_sync_value () = Esuccess (Vnat 3)) "a root that finishes does not answer its exit");
    theorem "w8_callback_fires" "Witnesses.lean:1022" [ ("w8Callback", w8_callback) ]
    ...
    theorem "w8_promise_squashes" "Witnesses.lean:1025" []
      (fun _ -> expect (w8_promise () = Error (Sq_error 4)) "`promiseOutcome` does not squash to the first `Fail` ahead of the defect");
    (* W13 -- R2-1 *)
    theorem "w13_completion_pop_sees_the_waiter_interrupt" "Witnesses.lean:1095" [ ("w13", w13) ]
      (fun d ->
        let s = d "w13" in
    ...
    theorem "w13_sync_meets_the_finalizer_program" "Witnesses.lean:1108" [ ("w13SyncUnderOnExit", w13_sync_under_on_exit); ("w13SyncUnderOnExitAnswered", w13_sync_under_on_exit_answered) ]
      (fun d ->
    ...
    (* W14 -- R2-3 *)
    theorem "w14_join_cleanup_drops_the_observer" "Witnesses.lean:1138" [ ("w14Parked", w14_parked); ("w14Cancelled", w14_cancelled) ]
      (fun d ->
    ...
    theorem "w3_host_interrupt_cancels_entrants" "Witnesses.lean:1166" [ ("w3HostParked", w3_host_parked); ("w3HostInterrupted", w3_host_interrupted) ]
      (fun d ->
    ...
    theorem "w3_settle_interrupts_the_parked_loser" "Witnesses.lean:1179" [ ("w3ParkedLoser", w3_parked_loser) ]
      (fun d ->
    ...
    theorem "forbidden_observers_and_double_exit" "Witnesses.lean:1190"
    ...
    theorem "forbidden_interrupt_while_running" "Witnesses.lean:1212" []
      (fun _ ->
        let m = setup () in
    ...
    theorem "forbidden_setInterruptible_as_current" "Witnesses.lean:1223" []
      ...
      (fun _ -> None);
    theorem "forbidden_no_notImplemented_defect" "Witnesses.lean:1231"
    ...
    theorem "forbidden_task_enqueued_during_drain" "Witnesses.lean:1247" [ ("w9AfterOneFire", w9_after_one_fire) ]
      (fun d ->
    ...
    theorem "w9_next_fire_runs_it" "Witnesses.lean:1253" [ ("w9AfterTwoFires", w9_after_two_fires) ]
      (fun d -> expect (exit_of (d "w9AfterTwoFires") 0 = Some (Esuccess (Vnat 1))) "the next fire did not run it");
    (* DB-07 *)
    theorem "db07_store_survives_failure" "Witnesses.lean:1270" [ ("dbSeven", db_seven) ]
      (fun d ->
        let s = d "dbSeven" in
