...
let step slots (st : step) : value =
  ...
  (* refs *)
  | "refMake" -> Effect.perform (Op_store (Rref_make (iof slots (a 0))))
  | "refGet" -> Effect.perform (Op_store (Rref_get (hof slots (a 0))))
  | "refSet" -> Effect.perform (Op_store (Rref_set (hof slots (a 0), iof slots (a 1))))
  | "refUpdate" -> Effect.perform (Op_store (Rref_update (hof slots (a 0), iof slots (a 1))))
  | "refModify" -> Effect.perform (Op_store (Rref_modify (hof slots (a 0), iof slots (a 1))))
  | "refGetAndSet" -> Effect.perform (Op_store (Rref_get_and_set (hof slots (a 0), iof slots (a 1))))
  | "refTryTake" -> Effect.perform (Op_store (Rref_try_take (hof slots (a 0), iof slots (a 1))))
  (* deferreds *)
  | "defMake" -> Effect.perform (Op_store Rdef_make)
  | "defSucceed" -> Effect.perform (Op_store (Rdef_succeed (hof slots (a 0), iof slots (a 1))))
  | "defFail" -> Effect.perform (Op_store (Rdef_fail (hof slots (a 0), iof slots (a 1))))
  | "defIsDone" -> Effect.perform (Op_store (Rdef_is_done (hof slots (a 0))))
  | "defPoll" -> Effect.perform (Op_store (Rdef_poll (hof slots (a 0))))
  | "defAwaitValue" -> Effect.perform (Op_store (Rdef_await_value (hof slots (a 0))))
  | "defAwaitError" -> Effect.perform (Op_store (Rdef_await_error (hof slots (a 0))))
  (* scopes *)
  | "scopeMake" -> Effect.perform (Op_store Rscope_make)
  | "scopeAdd" -> Effect.perform (Op_store (Rscope_add (hof slots (a 0), iof slots (a 1))))
  | "scopeRemove" -> Effect.perform (Op_store (Rscope_remove (hof slots (a 0), iof slots (a 1))))
  | "scopeClose" -> Effect.perform (Op_store (Rscope_close (hof slots (a 0))))
  (* layers *)
  | "layerBuild" -> Effect.perform (Op_store (Rlayer_build (iof slots (a 0))))
  | "layerProvideCount" -> Effect.perform (Op_store (Rlayer_provide_count (iof slots (a 0))))
  | "layerScopeOf" -> Effect.perform (Op_store (Rlayer_scope_of (hof slots (a 0))))
  | "layerClose" -> Effect.perform (Op_store Rlayer_close)
  (* terminals *)
