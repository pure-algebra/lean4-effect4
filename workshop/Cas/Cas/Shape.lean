...
(`markKey`), because that module imports the old store. `Json.ofNat` and `binary64OfNat` are
`src/Effect4/Store/JsonCanonical.lean:61-81`, copied for now as the plan says.
-/
...
/-- The binary64 bit pattern of a natural, exact below 2^53 and truncated toward zero above it
(`JsonCanonical.lean:69-78`). -/
def binary64OfNat (n : Nat) : UInt64 :=
  if n = 0 then 0
  else
    let e := highestBit n
    let significand :=
      if e ≤ 52 then (n <<< (52 - e)) - (1 <<< 52) else (n >>> (e - 52)) - (1 <<< 52)
    UInt64.ofNat (((1023 + e) <<< 52) + significand)

/-- A JSON number from a natural: the binary64 the host would parse (`JsonCanonical.lean:81`). -/
