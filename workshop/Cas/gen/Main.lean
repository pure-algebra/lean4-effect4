...
/-!
# gen.Main — the generator of `Canonical` instances
...
`fits` and the instance — exactly as `workshop/Cas/Cas/Templates.lean` and
`workshop/Cas/Cas/Program.lean` write them by hand.
...
other through `.named`.
    ...
    lake env lean -M 4096 --run workshop\Cas\gen\Main.lean --group Json \
      --imports Cas.Templates --out workshop\Cas\Gen-out\Json.lean \
      Effect4.Float64 Effect4.Json

The environment walk is `src/OCaml5/Tools/Describe.lean`'s (`getConstInfoInduct`,
`instantiateForall`, `forallTelescope`, `isStructure`, `InductiveVal.all`); the "generated Lean
text with a guard" pattern is `src/OCaml5/Lib/Derived.lean`'s.
...
- A recursive item reads through `guarded toVal raw` (`Cas/Canonical.lean`): `raw` is the
...
def emit (s : String) : EmitM Unit :=
  modify fun o => { o with lines := o.lines.push s.trimRight }

/-- `k` spaces. -/
def spaces (k : Nat) : String := String.mk (List.replicate k ' ')
...
def emitJoin (head : String) (items : List String) (sep : String) (tail : String)
  ...
  if items.isEmpty then
    ...
    for i in [0:items.length] do
      ...
      if line.length + chunk.length > 98 && !line.trim.isEmpty then
        emit line
        line := cont ++ chunk
      else
        line := line ++ chunk
    ...
    for i in [0:n] do
      ...
      if i + 1 == n then
        emit (ind ++ "(acceptsFields_cons _ _ _ _ _ _ (" ++ args[i]! ++
          ") (acceptsFields_nil _))" ++ String.mk (List.replicate (n - 1) ')'))
      else
        emit (ind ++ "(acceptsFields_cons _ _ _ _ _ _ (" ++ args[i]! ++ ")")
...
def emitRecursive (st : St) (ns : String) : EmitM Unit := do
  emit s!"namespace {ns}"
  emit ""
...
def emitPlain (st : St) (ns : String) : EmitM Unit := do
  let m := st.members[0]!
  let nf := st.foreigns.size
  emit s!"namespace {ns}"
  emit ""
  ...
  if isStructLike then
    ...
    -- unused `simp` argument; the same script is `Cas/Program.lean`'s `LitC.ofVal_toVal`.
...
partial def parseArgs : List String → Args → Args
  ...
  | t :: rest, a => parseArgs rest { a with types := a.types ++ [t] }
...
def run (args : Args) : MetaM (Array String) := do
  ...
  let head := "lake env lean -M 4096 --run workshop\\Cas\\gen\\Main.lean --group " ++ args.group
    ++ " --imports " ++ String.intercalate "," args.imports
    ++ " --out workshop\\Cas\\Gen-out\\" ++ args.group ++ ".lean"
  ...
  for s in seeds do
    ...
    let emitAct : EmitM Unit := if recursive then emitRecursive st ns else emitPlain st ns
...
def main (argv : List String) : IO Unit := do
  ...
  let env ← importModules
    #[{ module := `Cas.Templates }, { module := `Cas.Program },
      { module := `Effect4.Schema.Document }] {} 0
