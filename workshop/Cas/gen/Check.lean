...
/-!
# gen.Check — the projection guard on a generated file
...
dropped case a build failure rather than a silent change of every address in the store.
    ...
    lake env lean -M 4096 --run workshop\Cas\gen\Check.lean workshop\Cas\Gen-out\Program.lean

The comparison is deliberately *independent* of `gen/Main.lean`: this file reads the generated
text, not the generator's intermediate representation, and finds each carrier in the
...
or the bytes. Those are the emitted file's own receipts and its acceptance guards.
