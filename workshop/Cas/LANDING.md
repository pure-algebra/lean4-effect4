...
guards — lane B measured (2026-09-05) that `#guard` evaluates the compiled decision procedure,
not the kernel, so the SHA-256 of the 92 KB genesis decides in about two seconds and every
number is guarded outright; `WordContract`, `TraitContract` from `Cas/Probe.lean`;
`DerivedCheck` runs the projection guard's per-file entry point over the six derived files
and refuses a difference. `ArchContract`: `digestOf` → `Canonical.digest`, `rc112.find (digestOf …)`
