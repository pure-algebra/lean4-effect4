...
`E4-TARGET-CE-022`. A spelling outside the profile (a `Handle`'s target type)
...
Declaring an atom in one face only is `E4-TARGET-CE-023`. `OfVal` is the
...
`E4-TARGET-CE-022` and `E4-TARGET-CE-023`.
