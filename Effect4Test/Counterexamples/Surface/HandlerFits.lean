...
the ascribed slot definitions in `Effect4Test/Surface/HandlerFitsContract.lean`
...
… does **not** import `Effect4.Char.*`, `Effect4.Deep.*`, `Effect4.Syntax.*`,
or anything under `Effect4.Runtime`." Plan §13.2: "`Effect4/Surface/Handler.lean`
(wave 2d; imports `Effect4.Syntax.Typing` and `.Print`)". `EffTy` is
...
`tsAnswer : String`, rendered TypeScript text, while `Effect4.Syntax.Row`
...
end Effect4Test.Counterexamples.Surface.HandlerFits
