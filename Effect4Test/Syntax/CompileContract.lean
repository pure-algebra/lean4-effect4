...
`Effect4Test.*` at `propext`/`Quot.sound`, and Lean's UTF-8 folds reach `Classical.choice`.
...
namespace Effect4Test.Syntax.CompileContract
...
#guard exitOf (replayEff pForkJoin [evaluateRoot, fireRoot]) 0
  = Effect4.Deep.Witnesses.exitOf Effect4.Deep.Witnesses.w1DeferredJoin 0
...
end Effect4Test.Syntax.CompileContract
