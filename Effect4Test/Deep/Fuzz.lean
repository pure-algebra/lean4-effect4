...
Every battery under `Effect4Test/` before this one checks a handful of hand-written programs
at one tape. This module enumerates the *alphabet* instead: a bounded corpus of `ProgName`
programs over `Effect4.Deep.Stores`, a bounded corpus of `RunDecision` tapes, and nine
Boolean invariants over the machine `Effect4.Deep.Witnesses.replay` reaches on each pair.
...
namespace Effect4Test.Deep.Fuzz

open Effect4
open Effect4.Deep
open Effect4.Deep.Witnesses
...
"unimplemented step" defect; `Effect4.Deep.withFiber_forkScoped_none` is the clause. The
