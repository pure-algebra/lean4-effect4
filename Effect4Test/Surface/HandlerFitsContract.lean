...
`Effect4.Syntax.*` and `Effect4.Deep.*`, while plan §13.2 puts
...
namespace Effect4Test.Surface.HandlerFitsContract
...
def handlerBodySlot : Handler → Effect4.Syntax.Eff Nat := Handler.body
...
end Effect4Test.Surface.HandlerFitsContract
