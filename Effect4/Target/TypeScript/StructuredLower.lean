...
/-- The declarations a flow lowers to at the best form.

A flow with one declared root is the single exported declaration `lowerBest` has
always emitted, byte for byte (`lowerBest_single_root_eq`). A flow with two or
more takes the **dispatch** entry: entering a chosen root means entering the
dispatch loop at a chosen case, and the structured form has no case to enter at
-- its control is labelled blocks and `break`/`continue`. That is the same trade
`dispatch-fallback` already makes for an irreducible graph, and the census
records it through `entry-root`, which `ruleSet` reports for any multi-root
flow. -/
def lowerRootsBest (rows : ServiceRow) (program : FlowProgram) : Option (List Decl) :=
  if (program.flow.erase).roots.length ≤ 1 then
    (lowerBest rows program).map fun decl => [Decl.prog decl]
  else lowerRoots rows program

/-- **The byte-identity receipt at the best form.** A flow with one declared root
lowers through the multi-root emitter to exactly the single declaration
`lowerBest` emits -- structured where the graph is reducible, dispatch where it
is not. -/
theorem lowerBest_single_root_eq (rows : ServiceRow) (program : FlowProgram)
    (single : (program.flow.erase).roots.length ≤ 1) :
    lowerRootsBest rows program = (lowerBest rows program).map fun decl => [Decl.prog decl] := by
  simp only [lowerRootsBest, if_pos single]
...
/-- The declarations a region flow lowers to at the best form; the multi-root
case takes the dispatch entry, for the reason `Flow.lowerRootsBest` gives. -/
def lowerRootsBest (rows : ServiceRow) (program : RegionProgram) : Option (List Decl) :=
  if (program.flow.flow).roots.length ≤ 1 then
    (lowerBest rows program).map fun decl => [Decl.prog decl]
  else lowerRoots rows program

/-- **The byte-identity receipt at the best form, for region flows.** -/
theorem lowerBest_single_root_eq (rows : ServiceRow) (program : RegionProgram)
...
reducible or the flow declares more than one root), plain and region flows, over
...
def structuredModules? (families : List (ServiceRow × List FlowProgram × List RegionProgram))
  ...
  let decls ← families.mapM fun (rows, programs, regionPrograms) => do
    let lowered ← programs.mapM (Flow.lowerRootsBest rows)
    let loweredRegions ← regionPrograms.mapM (Region.lowerRootsBest rows)
