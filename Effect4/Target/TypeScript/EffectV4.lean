...
def constSource (row : AtomRow) : String :=
  "/** Host body of the pure atom `" ++ row.name ++ "`; its Lean model is `" ++
    row.name ++ "`. */\n" ++
  "export const " ++ row.name ++ " = (" ++ row.binder ++ ": " ++ row.tsRequest ++ "): " ++
...
def atomsModule (rows : List AtomRow) : String :=
  let needed := Spelling.namespacesOf (rows.flatMap fun row => [row.tsRequest, row.tsAnswer])
