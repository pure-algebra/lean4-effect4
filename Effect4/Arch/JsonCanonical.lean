...
def Representation.toJson? (value : Representation) : Option Json :=
  Target.TypeScript.Schema.reifyJson? (Target.TypeScript.Schema.representation value)

/-- The persisted JSON form of a document. -/
def Document.toJson? (document : Document) : Option Json :=
  Target.TypeScript.Schema.reifyJson? (Target.TypeScript.Schema.documentExpr document)
