...
* The Schema half: a persisted document or representation as its `Schema.Struct({…})`
  ...
  `Effect4.Target.TypeScript.Schema.generate?`, admitted by exact name in the axiom gate.

Everything else in the library is implementation behind this seam. Callers and tests cross
it the same way; `Effect4Test/Api/ApiContract.lean` is the receipt.
-/
...
def typeOf (program : Program) : Option EffTy := Syntax.typeOf nativeSignature program
...
def print (program : Program) : Except PrintRefusal TypeScript.Expr :=
  Syntax.print nativeSignature 0 program
...
def printDecl (name : String) (program : Program) : Option TypeScript.ConstDecl :=
  ...
  | some ty, Except.ok body => some (Syntax.printDecl name ty body)
  | _, _ => none
...
def compile (program : Program) (fuel : Nat) (choices : List Bool := []) : NCode :=
  Syntax.compile program fuel choices
...
/-- A persisted Schema document as its `Schema.Struct({…})` syntax. -/
def schemaDocument (document : Effect4.Document) : TypeScript.Expr :=
  Effect4.Target.TypeScript.Schema.documentExpr document

/-- A persisted representation as its Schema syntax. -/
def schemaRepresentation (representation : Effect4.Representation) : TypeScript.Expr :=
  Effect4.Target.TypeScript.Schema.representation representation

/-- A JSON payload as a TypeScript literal. -/
def jsonExpr (value : Effect4.Json) : TypeScript.Expr :=
  Effect4.Target.TypeScript.Schema.json value
