...
`Effect4.Target.TypeScript.Schema` emits the *persisted document* and decodes it
...
* an annotation payload with a `__proto__` key: `Target.TypeScript.Schema`
...
first-order JSON, so they spell exactly as `Target.TypeScript.Schema.json` does,
