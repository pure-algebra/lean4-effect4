...
| Generation | `Entity.tsModule` and `Domain.tsModule` (persisted, gated by `Target.TypeScript.Schema`), `Entity.tsConstructor` (the view of §4.2) |
...
def Entity.tsModule (dom : Domain) (entity : Entity) : Option TypeScript.Module :=
  Target.TypeScript.Schema.module? entity.name (entity.document dom)
...
def Domain.tsModule (dom : Domain) : Option TypeScript.Module :=
  if dom.wellFormed &&
      dom.entities.all (fun entity =>
        Target.TypeScript.Schema.generationReady entity.name (entity.document dom) []) then
    some
      { header := ["Generated by Effect4 Surface.", "", "Do not edit."]
        ...
        decls := dom.entities.flatMap fun entity =>
          [ Target.TypeScript.Schema.rawDocumentDecl entity.name (entity.document dom)
          , Target.TypeScript.Schema.documentDecl entity.name ] }
  else none
