...
| pFailText | fail [{"fail":"lost"}] | fail [{"fail":"lost"}] | yes | yes | yes | runSyncExit | runSyncExit built no fiber: a bare Exit is returned as is (`effectIsExit`, internal/effect.ts:5539); the compared schedule is the runFork entry's, which does start one; same fail payloads |
| pFailBoomText | fail [{"fail":"boom"}] | fail [{"fail":"boom"}] | yes | yes | yes | runSyncExit | runSyncExit built no fiber: a bare Exit is returned as is (`effectIsExit`, internal/effect.ts:5539); the compared schedule is the runFork entry's, which does start one; same fail payloads |
...
| pCatchIfMiss | fail [{"fail":7},{"die":{"user":3}},{"fail":9}] | fail [{"fail":7},{"die":"3"},{"fail":9}] | yes | yes | yes | runSyncExit | same fail payloads |
