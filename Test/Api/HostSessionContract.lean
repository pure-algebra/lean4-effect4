...
def header : Header := ⟨1, "session-A", "serial-root-scalar-v1", table⟩
...
def call0 : Call := ⟨1, "session-A", table, 0, Api.root, .external 0, .nat 2⟩
def bound0 : Session program table := (bindCall parked call0 0).session
def reply0 : Reply := ⟨1, "session-A", 0, .ofExit (.success (.nat 2))⟩
...
def reply1 : Reply := { reply0 with callId := 1, completion := .ofExit (.success (.nat 3)) }
...
#guard (applyPending pending0 0).session.pending = some reply0
...
#guard (submit bound1 reply0).phase = .refused .callOrder
...
#guard (bindCall parked { call0 with version := 2 } 0).phase = .refused .version
...
#guard (submit bound0 { reply0 with version := 2 }).phase = .refused .version
...
#guard (advance pending0 100 Api.evaluate).phase = .refused .pendingControl
