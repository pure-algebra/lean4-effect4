...
/-- The input family and configured joins determine the expected rows even if extraction fails. -/
def requiredRules (spec : RulesSpec) : CoreM (Array CheckId) := do
  let mut ids : Array CheckId := #[⟨"rules.extract", ⟨"algorithm",
    [spec.algorithm.name.toString, spec.algorithm.family.toString]⟩⟩]
  for (check, file) in [("rules.join.target", spec.heads), ("rules.join.second", spec.coverage)] do
    if let some f := file then ids := ids.push ⟨check, ⟨"file", [f.toString]⟩⟩
  if let some (.inductInfo info) := (← getEnv).find? spec.algorithm.family then
    for c in info.ctors do
      ids := ids.push ⟨"rules.join", ⟨"typingRule", [spec.algorithm.name.toString, c.toString]⟩⟩
  return ids
    ...
    let heads ← match spec.heads with
      ...
      | some f =>
        if ← System.FilePath.pathExists f then
          ...
          | .ok hs =>
            inputs := inputs.push f.toString
            rows := rows.push <| Row.pass "rules.join.target" ⟨"file", [f.toString]⟩ .tested "head table parsed"
            pure (some hs)
          | .error e =>
    ...
    let coverage ← match spec.coverage with
      ...
      | some f =>
        if ← System.FilePath.pathExists f then
          ...
          | .ok cs =>
            inputs := inputs.push f.toString
            rows := rows.push <| Row.pass "rules.join.second" ⟨"file", [f.toString]⟩ .tested "coverage table parsed"
            pure (some (coverageIndex cs))
          | .error e =>
