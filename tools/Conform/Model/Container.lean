...
structure Combinators (M : Type → Type) where
  ...
  /-- `none` when the selected library has no sum combinator. A plan reaching that node
  reports `representation.law-missing`. The Effect4 image adapter supplies `Image.sum`;
  its program-value adapter still requires a separately specified program sum type. -/
