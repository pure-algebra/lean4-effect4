...
**What it is.** The five words a conformance check may use for how a claim is supported, in
their strength order, and the four outcomes a check can report on one subject. Every row of
...
**Properties.**
* The order `assumed < stamped < tested < reproduced < proved` is total and is the only order
  on `Evidence` — *by construction* (`Evidence.rank`).
...
/-- The strength order, as a rank. -/
def rank : Evidence → Nat
  | .assumed => 0
  | .stamped => 1
  | .tested => 2
  | .reproduced => 3
...
instance : LT Evidence := ⟨fun a b => a.rank < b.rank⟩
instance : LE Evidence := ⟨fun a b => a.rank ≤ b.rank⟩
instance (a b : Evidence) : Decidable (a < b) := inferInstanceAs (Decidable (a.rank < b.rank))
instance (a b : Evidence) : Decidable (a ≤ b) := inferInstanceAs (Decidable (a.rank ≤ b.rank))
...
/-- The weaker of two grades: what a claim supported by both is worth. -/
def min (a b : Evidence) : Evidence := if a.rank ≤ b.rank then a else b
