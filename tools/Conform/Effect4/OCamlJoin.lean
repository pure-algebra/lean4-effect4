  ...
  let act : MetaM UInt32 := do
    ...
    let declReport : Conform.Report :=
      { tool := "conform.layout.ocaml-type-decl"
        ...
        expected := 4
        required := #["eff_types.ml", "api_gen.ml"].flatMap fun label =>
          #["ocaml.type-decl.structural", "ocaml.type-decl.bytes"].map fun check =>
            ⟨check, ⟨"declaration", ["ocaml-eff", "ty", label]⟩⟩
        rows }
    ...
    unless scanned.isEmpty do
      ...
      let auditReport : Conform.Report :=
        { tool := "conform.layout.audit[ocaml-eff recovered]"
          pins := [⟨"lean", "4.33.1"⟩, ⟨"target", T.name⟩]
          expected := (auditRequired tbl).size, required := auditRequired tbl, rows := auditRows }
