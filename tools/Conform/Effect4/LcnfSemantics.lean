...
def marshalRoundTrip : Array Row :=
  vectors.toArray.mapIdx fun i t =>
    let subj : Subject := { kind := "vector", path := ["marshal", toString i, t.render] }
  ...
  let act : CoreM UInt32 := do
    ...
    let report : Report :=
      { tool := "conform-lcnf-semantics"
        ...
        expected := vectors.length + cases.size
        required := (vectors.toArray.mapIdx fun i t =>
          ⟨"lcnf.semantics.marshal", ⟨"vector", ["marshal", toString i, t.render]⟩⟩) ++
          (cases.mapIdx fun i c => ⟨"lcnf.semantics.agrees", ⟨"case", [toString i, c.decl.toString, c.label]⟩⟩)
        rows := all }
