...
/-!
# Conform.Effect4.Typing.Inversion — every arm of every checker, inverted by the machine

For each arm of each of the six mutually recursive checkers of
`src/Effect4/Program/Typing.lean:176-435` there is one lemma here saying what must have held for
the arm to answer: *the arm's equation read as a proposition*. Together they are exactly the
premises the rules of `Conform.Effect4.Typing.HasTy` carry, and they are what the soundness
proof consumes.

**Nothing here is proved by hand.** Every lemma is the coordinator's pilot one-liner:

```lean
theorem inv_<arm> … : ∀ t, <checker> … = some t → <the arm as a proposition> := by
  refine Option.of_triple ?_
  simp only [<checker>]; mvcgen; all_goals simp_all
```

`harvest_specs` (below) writes the reflection specification of every `Option`-valued leaf the
checkers call; `simp only [f]` replaces the application by the arm's equation lemma; `mvcgen`
computes the weakest precondition through the binds, the guards and the matches; `simp_all`
closes the residue, whose hypotheses are the leaf equations and whose goal is the arm's
conclusion. `Option.of_triple` (`Conform.Spec.Reflect`) turns the triple back into an ordinary
implication, so no `wp` occurs in any statement here.

**One departure from the pilot's spelling, and it is a simplification.** `Probe4` wrote the
invariant twice — once as the theorem's conclusion and once as `(Inv := fun t => …)` — because
`Option.of_triple`'s `Inv` is not determined by `Inv t` for a fixed `t`. Stating the lemma in
the closed form `∀ t, f … = some t → P t` makes `?Inv` a Miller pattern, so `refine
Option.of_triple ?_` solves it: the invariant is written **once**, and a transcription slip
between the two copies is no longer possible. `refine Option.of_triple ?_ t h` on the pilot's
open form still fails (the metavariable is postponed, then reported unassigned).

**The deviations, counted.** Of the 68 lemmas below, **60 close with the one-liner unchanged**.
The other **eight** need one extra line, and it is the same line in all eight — the pilot's
`awaitFiber` finding, generalised. Whenever an arm binds a *pair* leaf (`let (value, error) ←
fiberTy t`, or `let _ ← fiberTy t`) and the pair's components do not appear on the surface of
the postcondition, the residual goal is the pure existential `∃ x y, p = (x, y) ∧ …`, and the
simplifier does not invent the witnesses: structure eta does. Two shapes, one idea:

| arms | residue | extra line |
| --- | --- | --- |
| `actionTy`: `runIn`, `interrupt`, `interruptScoped`, `interruptAll` (both) | `∃ x y, p = (x, y)` — the pair is *discarded* (`let _ ←`) | `all_goals exact ⟨_, _, rfl⟩` |
| `effTy`: `awaitFiber` at `.awaitValue`; `actionTy`: `awaitAll`, `awaitAllFailFast` | `∃ x y, p = (x, y) ∧ f p.1 p.2 = f x y` — the pair is used, but under a constructor | `all_goals exact ⟨_, _, rfl, rfl⟩` |

`awaitFiber` at `.joinEffect` closes with the one-liner (its components are the postcondition's
own columns, so `simp_all` reads the witnesses straight off). The eight were confirmed minimal
by deleting every extra line and rebuilding: exactly those eight go red, at
`Option.of_triple`'s residue and nowhere else. **No arm defeated the generator**; there is no
hand-written inversion, and no `Option.bind_eq_some_iff` chain, anywhere in this file.
-/
...
/-! ## The leaf specifications, generated

One `harvest_specs` per checker. The commands are idempotent, so the overlap between the six
checkers' leaf sets costs nothing: `effTy` contributes ten, and the other five add
`layersTy`, `litVal`, `GenTy.merge` and `effsTy`. -/

harvest_specs Effect4.Program.effTy
harvest_specs Effect4.Program.layerTy
harvest_specs Effect4.Program.layersTy
harvest_specs Effect4.Program.stmtsTy
harvest_specs Effect4.Program.effsTy
harvest_specs Effect4.Program.actionTy
...
theorem inv_callback (sig : Signature Op) (env : TyEnv) (register : Op) (request : Term) :
    ∀ t, effTy sig env (.callback register request) = some t →
      sig.dom register = true ∧ (sig.rowOf register).kind = .async ∧
...
theorem inv_provideService (sig : Signature Op) (env : TyEnv) (key : ServiceKey) (value : Term)
    ...
    ∀ t, effTy sig env (.provideService key value body) = some t →
      ∃ ty b, sig.serviceTy key = some ty ∧ termTy sig env value = some ty ∧
