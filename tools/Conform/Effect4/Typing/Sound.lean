import Conform.Effect4.Typing.HasTy
import Conform.Effect4.Typing.Inversion
...
theorem effTy_sound (sig : Signature Op) (e : Eff Op) :
  ...
  | yieldError error =>
    intro env t h
    obtain ⟨ty, hty, rfl⟩ := inv_yieldError sig env error t h
    exact .yieldError hty
  | sync thunk =>
    intro env t h
  ...
  | callback register request =>
    intro env t h
    obtain ⟨hdom, hkind, hreq, rfl⟩ := inv_callback sig env register request t h
    exact .callback hdom hkind hreq
  ...
  | «scoped» body =>
    intro env t h
    exact HasTy.scoped (effTy_sound sig body env t (inv_scoped sig env body t h))
  ...
  | provideService key value body =>
    intro env t h
    obtain ⟨ty, b, hty, hval, hb, rfl⟩ := inv_provideService sig env key value body t h
    exact .provideService hty hval (effTy_sound sig body env b hb)
