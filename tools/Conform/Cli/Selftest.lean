...
| the weakest evidence of a report is the minimum over its passing rows | `weakest` |
-/
...
def report (rows : List Row) (expected : Nat := rows.length) : Report :=
  { tool := "selftest", expected, rows := rows.toArray }
...
#guard (report []).exitCode = 0                                     -- zero expected, zero rows

-- weakest evidence over passing rows only
#guard (report [passRow "a" .proved, passRow "b" .stamped, refusedRow "c"]).weakestEvidence = some .stamped
#guard (report [refusedRow "c"]).weakestEvidence = none

-- the evidence order is the documented one
#guard Evidence.assumed < Evidence.stamped ∧ Evidence.stamped < Evidence.tested ∧
  Evidence.tested < Evidence.reproduced ∧ Evidence.reproduced < Evidence.proved
#guard (Evidence.min .proved .tested) = .tested
