...
def run (args : Args) (cfg : Config) : CoreM (Report × Option Json) := do
  ...
  | some file =>
    ...
    acc := { acc with
      ...
      required := acc.required ++ Conform.Lcnf.required scan policy
  ...
  | some file =>
    ...
    acc := { acc with
      rows := acc.rows ++ rows
      expected := acc.expected + expected
      required := acc.required ++ Conform.Manifest.required spec
  ...
  if args.withRules then
    ...
    | none =>
      acc := { acc with
        ...
        required := acc.required.push ⟨"rules.extract", ⟨"config", ["rules"]⟩⟩ }
    | some file =>
      let spec ← Conform.Lcnf.RulesSpec.load file
      let required ← Conform.Lcnf.requiredRules spec
      let (table, rows, _) ← Conform.Lcnf.extract spec
      rules? := some table
      acc := { acc with
        rows := acc.rows ++ rows
        expected := acc.expected + required.size
  ...
  if acc.expected == 0 then
    acc := { acc with
      ...
      expected := 1
      required := #[⟨"conform.run", ⟨"config", [args.config.toString]⟩⟩] }
  let report : Report :=
    { tool := cfg.tool, pins := acc.pins, inputs := acc.inputs
    , expected := acc.expected, required := acc.required, rows := acc.rows }
  return (report, rules?)
