...
* **Idempotent.** An existing name is checked against the exact proposition and axiom policy, then skipped with a note, so `harvest_specs` may be
...
def declareReflect (f : Name) : TermElabM (Except Refusal Name) := do
  ...
  let result ← forallTelescope info.type fun xs body => do
    ...
    if (← getEnv).contains target then
      let expected : Conform.ProofRef := ⟨target, info.levelParams, stmt⟩
      if let .error why ← expected.validate then throwError why
      return .error (Refusal.exists_ target)
...
def optionLeaves (es : Array Expr) : MetaM (Array Name) := do
