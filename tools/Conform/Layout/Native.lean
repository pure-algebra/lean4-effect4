...
def required (profile : String) (W : World) (natives : Array TypeNative) : Array CheckId :=
  (#["native.tag", "native.fields"].flatMap fun check => W.types.flatMap fun tv =>
    if (find? natives tv.name).isNone then
      #[⟨check, ⟨"type", [profile, tv.name.toString]⟩⟩]
    else tv.ctors.toArray.map fun cv => ⟨check, ctorSubject profile tv.name cv.name⟩) ++
  (natives.flatMap fun t => t.ctors.map fun c => ⟨"native.limits", ctorSubject profile t.type c.ctor⟩)
