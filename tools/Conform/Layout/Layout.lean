...
def rule? (T : Target) (n : Name) : Option Rule :=
  (T.rules.find? fun r => r.type == n && r.applies.isNone) <|> T.rules.find? (·.type == n)
  ...
  let matching := T.rules.find? fun r =>
    r.type == h && (match r.applies with
      | some args => args.length == ty.args.length &&
          (args.zip ty.args).all fun (a, b) => a.render == b.render
      | none => false)
  match matching with
  | some r => pure r
  | none => T.rule? h
...
def reader (j : Json) : Policy.Reader Rule := do
  ...
  pure { type, discrimination := d, ctors := ctors.toList, scalar, container,
