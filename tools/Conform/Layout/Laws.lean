...
abbrev Injective {α : Type} (f : α → TVal) : Prop := Function.Injective f

theorem injective_of_explicit {α : Type} {f : α → TVal}
    (h : ∀ a b, f a = f b → a = b) : Function.Injective f := fun {a b} => h a b

theorem injective_to_explicit {α : Type} {f : α → TVal}
    (h : Function.Injective f) : ∀ a b, f a = f b → a = b := fun a b heq => @h a b heq

theorem Injective.id : Function.Injective (fun v : TVal => v) := fun _ _ h => h
...
theorem nullable_injective {α : Type} (e : α → TVal) (he : Function.Injective e)
    (hnull : ∀ a, e a ≠ TVal.null) : Function.Injective (nullable e) := by
  intro a b h
  match a, b with
  | none, none => rfl
  ...
  | some x, some y => exact congrArg some (he h)
...
theorem nativeOption_injective {α : Type} (noneC someC : String) (e : α → TVal)
    (he : Function.Injective e) (hne : noneC ≠ someC) : Function.Injective (nativeOption noneC someC e) := by
  intro a b h
  match a, b with
  | none, none => rfl
...
theorem map_injective {α : Type} (e : α → TVal) (he : Function.Injective e) :
  ...
  | x :: xs, y :: ys, h => by
    ...
    injection h with h1 h2
    rw [he h1, map_injective e he xs ys h2]

theorem seq_injective {α : Type} (e : α → TVal) (he : Function.Injective e) : Function.Injective (seq e) := by
  intro xs ys h
  unfold seq at h
  injection h with h
