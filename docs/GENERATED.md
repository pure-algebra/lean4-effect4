...
| --- | --- | --- | --- | --- | --- |
| derived | `tools/Effect4Gen/Driver.lean` runs `Main.lean` / `Fold.lean` per manifest group: `Json`, `Schema`, `Program`, `Pin`, `Api`, `Fold`, `SchemaFold` | `tools/Effect4Gen/manifest.json`, the guards, the imports named per group | the Effect4 library (`Store/Derived/*.lean`, `Program/Derived.lean`, `Store/PinDerived.lean`, `Api/Derived.lean`, `Program/Fold.lean`, `Schema/Fold.lean`) | `make check-gen`; `lake build Test` for shapes | reproduced; tested |
