...
**`whileLoop` is not retired; it waits for an owner decision.** Its typing lemma is proved and stays in `Laws/Program/Retire.lean` (`effTy_iterate_of_whileLoop`: a typed `whileLoop` has the same type as `iterate` at the initial cursor's type with a unit result; the converse is false on purpose, since `iterate` admits a step under the cursor's type). The deletion is blocked by reading, not by a proof. `iterate` prints with a cursor annotation (`let aN: T = initial`) and neither reader reads it (`readable` is false until R5). B19 rules that no `TypeRef → Ty` inverse exists, and the reader has no typing environment from which to recompute the annotation. Deleting `whileLoop` now would make every loop unreadable until R5: `Api.read`, `admitModule` and both readers would refuse them, and the loop programs would leave the round trip lane (45 of the 410 corpus programs hold a `whileLoop` today, 28 of the 284 readable ones). The question and a recommendation are in `docs/STATE.md`.
