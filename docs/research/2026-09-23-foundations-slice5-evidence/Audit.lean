...
run_cmd do
  ...
  for (name, _) in env.constants.toList do
    if let some idx := env.getModuleIdxFor? name then
      if env.header.moduleNames[idx.toNat]? == some `Test.Counterexamples.Machine.Semantics.AsyncHookContract then
  ...
  logInfo m!"COUNTEREXAMPLE MODULE: {count} declarations checked at [propext, Quot.sound]"
