...
| §3: row 85 as proposed; `Array → list` in the LCNF route; the OCaml store is native and property-tested against the Lean lists | **Confirmed.** `src/OCaml5/Lcnf/Types.lean:34`, `Translate.lean:241` (`Array.push` → `@ [x]`; the review's `:574-577` is `listFact?`), `ocaml/engine/e4_store.ml` (not `engine/src/`), `ocaml/engine/test/prop_store.ml` ST1–ST8 against `Stores.lean`'s list operations. The five law names are the review's proposal. Row 85 unchanged; the ruling is the owner's. **Qualified by the follow-up (§11):** `e4_store.ml` wraps `E4_table`, whose nonempty representation is `Map.Make(Int)` (`e4_table.ml:21-28`), and Lean's allocation appends; the review's cost claims (constant-time prepend, native indexed storage) are rejected, and neither the host property tests nor the cost story is evidence of an array or of a verified lowering. The law interface stands on its own. |
...
| §5.3: the deletions | **Accepted, one corrected.** `DeferredOk`'s two conjuncts are both `CompletionShaped` (`Simulation/Hooks.lean:33-35`), so the whole predicate goes and `StoresOk` keeps `ScopeKeysFresh` alone; plan §14's "`DeferredOk.1`" is corrected. `denoteStored`'s five uses (`InterpR.lean:348,355,359`; `Simulation/Drive.lean:81,192`) go with it; `STORES-FB-COMPLETION` goes; `E4-STORES-CE-003` (`Test/Machine/Runtime/StoresLawsContract.lean:80`, `Test/Counterexamples/REGISTER.md:131`) does **not** (§11): its `Completion.ofRefGet ⟨9⟩` on a reference that does not exist is a validity boundary the completion data leaves open, so it is translated to the data representation and stays until the promise/handle invariant and its preservation theorem discharge it. |
...
## 11. The refinement follow-up, addressed (2026-09-19)

`docs/research/2026-09-19-refinement-followup.md` (`60b7d0da`, three read-only seats and a
coordinator) reviews `f4404923` through `65b143a2` with witnesses under
`docs/research/2026-09-19-refinement-followup/`. Each finding and its disposition; the owner's
same-night ruling on the gates (drop the ratchet and the source scan, collapse the tiers, cut the
scripts that change no M1–M7 decision) is recorded here because it settles two of them.

| finding | disposition |
| --- | --- |
| §1 P1: coverage followed types, not fields — a custom predicate on one field exempted every occurrence of the child's type; a column counted as owned if any emitted predicate bore its name | **Fixed** in `src/Effect4/Laws/Auto/TypedStateDecl.lean`: `Emit.skipped` (child types) is `Emit.covered` (field keys), the accounting walks the owners on a path of uncovered edges from the roots, and a column owner's clauses account for exactly the position keys its `columnsUnder` walk reached (`columnKeysUnder`), so the check is per occurrence. The follow-up's two probes are controls `SkippedOccurrence` and `ColumnOwnership` in `Test/Audit/TypedStateDecl.lean`, each expecting the refusal by key. The real skeleton is unchanged. |
| §2 P1: completion data does not establish reference validity; do not delete `E4-STORES-CE-003` | **Accepted** (§10 and plan §14 amended). `Completion.ofRefGet ⟨9⟩` in a valid Deferred with no reference 9 satisfies `Stores.WF`; the counterexample is translated to the data representation as the reference-validity boundary and stays until the promise/handle invariant discharges it. `DeferredOk` still goes whole. |
| §3 P1: the proved budget is `straight_sufficient`'s, from `loadR` with a single root and empty stores; an embedded attempt needs a connector | **Accepted** (row 84 amended, still open). The bound and the driver lemmas are reused; the `tx` wrapper owes an obligation for its outer continuation and stores, attempt entry and exit work, the reserved budget and isolation, under which the embedded attempt inherits the bound. A number computed at admission is not the driver's remaining work. |
| §4 P2: the map's Tools-below-OCaml5 rule contradicts `docs/ARCHITECTURE.md`; presence is not completion; local empty-directory state in a tracked page | **Accepted.** The rule stands as the question the accepted Tools split answers (§10, not landed). The stack's caption now says a solid box is a present source, with completion recorded by plan §14 and the ledger; the empty-directory audit is printed by the driver and no longer rendered into the tracked page. |
| §5 P2: retain cheap counterchecks for the parser scanner and the ceiling; `check-known-red.sh` has no caller | **Superseded by the owner's ruling.** The proof-shape ceiling (row 62) and the source scan are retired: the gate reads the compiled environment only, and the `unsafe`/`partial`/axiom/`@[extern]`/`@[implemented_by]`/bodyless-`opaque` refusals are declaration-level; a `sorry` inside an `example` is a build warning until `-DwarningAsError=true` lands on a warning-free tree (237 today). The known-red mechanism, its script and its list are deleted (the list was empty). |
| the rejected claim: constant-time prepend and native indexed OCaml storage | **Accepted**; §10's row 85 is qualified. |

**The gates after the ruling.** `make check` is the build (the axiom audit inside it), the fresh
root elaboration and the generated-file drift; `make check-full` is everything else. Deleted:
the gate's self-test and its two harnesses, the tokenizer harness, the citation scanner with its
allow-list and internal-citations gate, the compatibility lane (`tools/Compatibility`, its script,
library, test and baseline fixtures), the tests of the conform-report, program-structure and
corpus-check scripts, the known-red script and library, the proof-shape pin and every trust-gate
fixture but the boundaries one. Kept: the generators, the host oracles (demoted to `check-full`)
and the one four-second boundaries harness.
