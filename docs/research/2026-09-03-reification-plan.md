...
| D2 | L1/L2 against `Scope`; region runner/denotation and plain erasure laws; fuel-free `RegionTotal` agreement with independent body review; `RegionSafety` checked-entry non-stuck theorem, wire/fuel-free corollaries, matching-frame/parent-chain proof, 31 frozen controls and three reviewed axiom receipts | local D2 fuel-free and reachable-stack obligations closed; general finalizer-machine connection remains D4, and host/interrupt meaning stays separately open |
| D3 | skeleton IR and T3 dispatch equation; actual order/computed lexical binding; general `StructureSemantics` T4 at `1ca39de`, frozen packet `33409c6`: every successfully emitted ordinary Flow with interrupts disabled, arbitrary merges/loops, parameter moves and every tape/input, at sufficient target fuel; 40 controls and two independently reviewed Program equations | ordinary-Flow T4 is closed at the stated Program face. Region/interrupt-point denotation, arbitrary smaller-fuel equality, rendering and host agreement remain open. Computed lexical facts do not assert independent all-path/nearest-dominator correctness |
| D4 | fence A; `compile_simulates`; restricted `unwind_failure`/`close_success` and three evaluated region instances; boundary amendment at `623141b`; independent `ScopeMachine` request/response stepping, ten local universal proofs and unchanged 27+72 controls, packet `e5a2bdb` | the unrestricted region connection remains refuted by `E4-TARGET-CE-019`..`021`; the local Scope close component now retains every prefix and agrees with the existing completed fold. The replacement compiler and general finite-prefix/residual-state simulation, arbitrary finalizer programs, interruption, parallel scheduling and host connection remain open |
...
| M2 | interrupt decisions and point-local deferral; real-mask counterexamples at `3fbf47d`; completed ScopeMachine-to-FrameFiber restoration at packet `945f729`: nine universal local laws, 56 guards, RUN-025/026 and 40 source-pinned host runs distinguish successful restoration, existing failure, nested masks and ordered cleanup causes; runner restoration repair, real mask lowering (`region-masked`, typescript v0.4.2) and host delivery through rc.112's own mask: the three interrupt goldens agree under every mask and `InterruptMaskBoundary` is green | the Flow result lacks full Cause/interrupt identity; nested/failing-exit rules beyond RUN-025; `interruptPoint` in the skeleton denotation |
