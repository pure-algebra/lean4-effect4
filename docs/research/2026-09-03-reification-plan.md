...
| D2 | L1/L2 against `Scope` (`Effect4/Flow/Region.lean`); `runRegions_eq_interpret`, `runRegions_erase` (`Effect4/Semantics/RegionDenotation.lean`); continuation: `denoteRegionsWF`, `denoteRegionsFuel_eq_denoteRegionsWF` and `runRegionsCause_eq_interpretWF` (`Effect4/Semantics/RegionTotal.lean`), with independent body review and four axiom receipts | the stack invariant making the empty-stack arms unreachable; the fuel-free continuation does not assume it |
