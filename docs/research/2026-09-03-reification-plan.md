...
| D3 | skeleton IR; T3 at every fuel and T4 on the flat fragment; `StructureOrder` actual order/conditional binding proof; `StructureDominators` derives both computed placement facts and proves strict binding of every successfully lowered body without supplied graph assumptions | full T4 merge/loop destination resumption, parameter moves, semantic composition and sufficient fuel; interrupt-point denotation. Computed lexical facts do not assert independent all-path/nearest-dominator correctness |
...
| M2 | interrupt decisions and point-local deferral laws; original three historically passing host fixtures; independent real-mask counterexamples and ten host runs at `3fbf47d` | correct restoration before another point or immediate return (three frozen guards currently red), nested/failing-exit rules, actual first-order mask lowering and immediate host delivery; historical masked golden must be amended as frozen |
