...
#proof_wanted M1.deferred_register_sites
...
#proof_wanted M1.registerAsync_sites
...
def M1.registerAsync_result_sites (p : NativeEff) (completed : List (FiberId × ExitV))
    ...
    (h : ((interpAt p completed table).registerAsync name fid token state).2 = some code) : ProofGraph.Obligation (
    raceSites code = []) := ⟨⟩
#proof_wanted M1.registerAsync_result_sites
...
def M1.evaluatePrim_owned (p : NativeEff) (completed : List (FiberId × ExitV)) (table : RowTable)
    ...
    (bounds : RaceIdsBelow m) (hf : FrameCodeOwned m f) : ProofGraph.Obligation (
...
#proof_wanted M1.evaluatePrim_owned
...
#proof_wanted M1.evaluateNative_frameOwned
...
#proof_wanted M1.iteration_frameOwned
