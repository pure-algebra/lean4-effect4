...
def M1Origin.beginRace_owned (p : NativeEff) (completed : List (FiberId × ExitV)) (table : RowTable)
    ...
    (bounds : RaceIdsBelow m) (hf : FrameCodeOwned m f) (site : Option (List Nat) := none)  : ProofGraph.Obligation (FrameCodeOwned (beginRace (interpAt p completed table) m f yielding entrants site).machine
      (beginRace (interpAt p completed table) m f yielding entrants site).fiber) := ⟨⟩
#proof_wanted M1Origin.beginRace_owned
...
def M1.registerAsync_result_sites (p : NativeEff) (completed : List (FiberId × ExitV))
    (table : RowTable) (name : EffName) (fid : FiberId) (token : Nat) (_state : Stores) (code : NCode)
...
theorem evaluatePrim_owned (p : NativeEff) (completed : List (FiberId × ExitV)) (table : RowTable)
  ...
  unfold evaluatePrim
  aesop (rule_sets := [Effect4.Stores])
    (add safe [stepFrame_owned, finalizerOr_owned, registerRace_owned, countdownPark_owned,
      withFiber_owned, withFiber_hook_sites, frameCodeOwned_same_races,
      registerAsync_result_sites, exitValue_sites, replace_current_sites])
...
theorem exitScoped_owned (p : NativeEff) (m : NativeMachine) (f : NFiber)
  ...
  all_goals
    aesop (rule_sets := [Effect4.Stores])
      (add safe [evaluatePrim_owned, frameCodeOwned_same_races, getCont_frameSites,
        replace_current_sites, raceSites_ofExit, raceSites_closeScopeUnsafe])
...
theorem evaluateNative_frameOwned (p : NativeEff) (table : RowTable)
  ...
  aesop (rule_sets := [Effect4.Stores])
    (add safe [evaluatePrim_owned, exitScoped_owned, frameCodeOwned_same_races])
