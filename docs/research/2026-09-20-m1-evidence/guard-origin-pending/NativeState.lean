...
def M1.StateStep.store {m : NativeMachine} (state : GuardState m) (stores : Stores)
...
def M1.StateStep.storeFresh {m : NativeMachine} (state : GuardState m)
...
theorem guardState_appendRace {m : NativeMachine} (state : GuardState m)
  ...
  · simp only [List.map_append, List.map_cons, List.map_nil, List.nodup_append]
    refine ⟨state.raceIds, List.nodup_singleton _, ?_⟩
...
def M1Origin.StateStep.beginRace (p : NativeEff) (table : RowTable) (completed)
    ...
    (programs : ∀ code ∈ entrants, raceSites code = []) (site : Option (List Nat) := none)  : ProofGraph.Obligation (StateStep m (beginRace (interpAt p completed table) m f yielding entrants site).machine) := ⟨⟩
...
def M1Origin.StateStep.spawn (p : NativeEff) (table : RowTable) (completed)
    ...
    (sites : raceSites code = []) (options : Supervision.ForkOptions) (site : List Nat := [])  : ProofGraph.Obligation (StateStep m (spawn (interpAt p completed table) m f code options site).1) := ⟨⟩
