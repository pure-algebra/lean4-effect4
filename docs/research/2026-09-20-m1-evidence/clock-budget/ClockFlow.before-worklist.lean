...
private structure CallableState where
  values : Std.HashMap ValueKey (Array Origin) := {}
  facts : Nat := 0
...
private def addOrigins (s : CallableState) (key : ValueKey) (values : Array Origin) : CallableState :=
  values.foldl (fun s value =>
    ...
    if old.contains value then s else
      { s with values := s.values.insert key (old.push value), facts := s.facts + 1 }) s

private def copyOrigins (s : CallableState) (to source : ValueKey) : CallableState :=
  addOrigins s to (s.values.getD source #[])
...
private def resolveCallables (env : Environment) (roots : Array Name) (flows : Array Flow) :
  ...
  let applyTarget (s : CallableState) (owner : Name) (result : Option FVarId)
    ...
    for i in [:args.size] do
      if let some parameter := params[supplied + i]? then
        if let .fvar argument := args[i]! then s := copyOrigins s parameter (owner, argument)
    if let some result := result then
      ...
      if supplied + args.size < arity then
        ...
        else for value in returns do s := copyOrigins s result value
    return s
  ...
  for _ in [:sites * origins + 1] do
    ...
    for flow in flows do
      for (to, source) in flow.aliases.toArray do
        state := copyOrigins state (flow.name, to) (flow.name, source)
      for (id, name, args) in flow.globals.toArray do
      ...
      for (result, source, expected, index) in flow.projections do
        for origin in state.values.getD (flow.name, source) #[] do
          match origin with
          | .unresolved => state := addOrigins state (flow.name, result) #[.unresolved]
          | .constructor site =>
              ...
              if expected.isNone || expected == some ctor then
                if let some (.ctorInfo ci) := env.find? ctor then
                  if let some (.fvar field) := args[ci.numParams + index]? then
                    state := copyOrigins state (flow.name, result) (site.1, field)
          | .callable .. => pure ()
      for (callee, args, result) in flow.localCalls do
        for origin in state.values.getD (flow.name, callee) #[] do
          match origin with
          | .callable target supplied =>
            state := applyTarget state flow.name result target supplied args
          | .unresolved =>
            if let some result := result then
              state := addOrigins state (flow.name, result) #[.unresolved]
          | .constructor _ => pure ()
    if state.facts == before then
      converged := true
      break
  unless converged do return none
  ...
  for flow in flows do
    ...
    for (callee, args, result) in flow.localCalls do
      let choices := state.values.getD (flow.name, callee) #[]
      if let some result := result then
