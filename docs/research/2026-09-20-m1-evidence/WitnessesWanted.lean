...
theorem w6_sequential_captures_and_merges :
    exitOf w6Sequential 0 =
      ...
      fiberCount w6Sequential = 1 ∧ scopeClosed w6Sequential 3 = some true :=
  by aesop
...
theorem w10_program_masks_its_own_fiber :
    exitOf w10MaskedPending 1 = none ∧
      ...
      exitOf w10Masked 1 = some (interruptedBy ⟨0⟩ ⟨1⟩) :=
  by aesop
    ...
    exitOf w12AwaitAll 0 =
      some (Exit.success (stores.exitsValue
        [Exit.success (Val.nat 4),
          Exit.failure (Cause.fail (Err.tag 5))])) :=
  by aesop
...
theorem w7_set_answers_the_cell :
    exitOf w7Set 0 = some (Exit.success (Val.cell ⟨0⟩)) ∧ w7Set.state.refs = [Val.nat 5] :=
  by aesop

/-- `ref.update`: the same `void`-declared shape succeeds with `undefined`, and the function is
applied exactly once. -/
theorem w7_update_answers_void :
    exitOf w7Update 0 = some (Exit.success Val.unit) ∧ w7Update.state.refs = [Val.nat 2] :=
  by aesop
...
theorem w7_modify_some_writes_back_the_read :
    exitOf w7ModifySome 0 = some (Exit.success (Val.nat 1)) ∧
      w7ModifySome.state.refs = [Val.nat 1] :=
  by aesop
...
theorem w7_update_some_and_get_rereads :
    exitOf w7UpdateSomeAndGet 0 = some (Exit.success (Val.nat 0)) ∧
      exitOf w7GetAndUpdateSome 0 = some (Exit.success (Val.nat 3)) :=
  by aesop
...
theorem w8_sync_exit_async_fiber_error :
    w8SyncExit = Exit.failure (Cause.die Defect.asyncFiber) :=
  by aesop
...
theorem w8_sync_exit_value : w8SyncValue = Exit.success (Val.nat 3) :=
  by aesop

/-- `runCallback` fires the callback observer with the root's exit under its key. -/
theorem w8_callback_fires : callbackRows w8Callback = [(77, [0, 1, 3])] :=
  by aesop

/-- `promiseOutcome` squashes a failure to its first `Fail`, ahead of the defect. -/
theorem w8_promise_squashes : w8Promise = Except.error (Squashed.error (Err.tag 4)) :=
  by aesop
...
theorem w15_flush_runs_callbacks_in_arming_order :
    armedQueueOf w15Armed = [2, 1] ∧
      ...
      resumeAndExitOrder w15Flushed = [[1, 0], [0, 1, 0], [0, 2, 1], [1, 2], [0, 1, 2], [1, 1]] :=
  by aesop
...
theorem w13_sync_meets_the_finalizer_program :
    finalizerRuns w13SyncUnderOnExit 0 = 1 ∧
      ...
      finalizerRuns w13SyncUnderOnExitAnswered 0 = 1 :=
  by aesop
...
theorem w3_settle_interrupts_the_parked_loser :
    exitOf w3ParkedLoser 0 = some (Exit.success (Val.nat 2)) ∧
      ...
      raceRows w3ParkedLoser = [[0, 0, 1], [0, 0, 2], [2, 0]] :=
  by aesop
...
theorem forbidden_setInterruptible_as_current (flag : Bool)
    ...
    (FrameFiber.step stores.toPrimInterp { frame with current := Prim.setInterruptible flag }).1 =
      FrameStep.running
        { frame with current := Prim.failure (Cause.die Defect.notImplemented) } :=
  by aesop
...
theorem forbidden_task_enqueued_during_drain :
    exitOf w9AfterOneFire 0 = none ∧ armedOf w9AfterOneFire 0 = some true ∧
      queuedOf w9AfterOneFire 0 = some 1 :=
  by aesop

/-- The next `fire` runs it. -/
theorem w9_next_fire_runs_it :
    exitOf w9AfterTwoFires 0 = some (Exit.success (Val.nat 1)) :=
  by aesop
