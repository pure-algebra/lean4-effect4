...
#proof_wanted M1Origin.StateStep.beginRace
...
theorem registerAsync_state (p : NativeEff) (table : RowTable) (completed)
  ...
  cases name <;> aesop (rule_sets := [Effect4.Stores]) (add safe [deferred])
...
theorem syncState_state (p : NativeEff) (table : RowTable) (completed)
  ...
  all_goals
    aesop (rule_sets := [Effect4.Stores]) (add safe [StateStep.store, syncOpStep_storeKeys])
...
#proof_wanted M1Origin.StateStep.spawn
...
#proof_wanted M1Results.registerAsync
...
#proof_wanted M1Results.countdown
...
theorem evaluatePrim_state (p : NativeEff) (table : RowTable) (completed)
  ...
  aesop (rule_sets := [Effect4.Stores])
    (add safe [StateStep.refl, StateStep.emit, StateStep.arm, StateStep.grow,
      StateStep.registerRace, StateStep.countdown, StateStep.joinPark,
      withFiber_state, stepFrame_state, finalizerOr_state, syncState_state,
      StateStep.storeFresh, registerAsync_state, ha])
...
#typed_state_obligations Effect4.Program.Guard.NativeState.M1 ceiling 0 using
  aesop (rule_sets := [Effect4.Stores]) (add safe [StateStep.store, StateStep.storeFresh, registerAsync_state])
