...
def M1.guardState_withState {m : NativeMachine} (state : GuardState m) (stores : Stores)
    (keys : storeKeys stores ⊆ storeKeys m.state) : ProofGraph.Obligation (
    GuardState { m with state := stores }) := ⟨⟩
#proof_wanted M1.guardState_withState
...
def M1.codeSites_dueResumes (p : NativeEff) (table : RowTable) (m : NativeMachine) : ProofGraph.Obligation (
    ∀ owed ∈ ((interpOf p table).dueResumes m.state).1,
      raceSites owed.code = []) := ⟨⟩
#proof_wanted M1.codeSites_dueResumes
...
def M1.guardState_withStoreKeys {m : NativeMachine} (state : GuardState m)
    (stores : Stores) (keys : ReservedKeys m (storeKeys stores)) : ProofGraph.Obligation (
    GuardState { m with state := stores }) := ⟨⟩
#proof_wanted M1.guardState_withStoreKeys
...
def M1.guardState_storeFresh {m : NativeMachine} (state : GuardState m)
    ...
    (keys : storeKeys stores ⊆ storeKeys m.state ++ [(fiber, m.nextToken)]) : ProofGraph.Obligation (
    GuardState { m with state := stores, nextToken := m.nextToken + 1 }) := ⟨⟩
#proof_wanted M1.guardState_storeFresh
