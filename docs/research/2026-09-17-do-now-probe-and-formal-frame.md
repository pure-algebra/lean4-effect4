...
| `Endpoint.lean` | 383 | `Endpoint`/`ApiSpec`/`SchemaFn`/`SchemaTransform`: a second authoring plane ("fluent metadata and typed ports to machine `Row`s"), written before `ServiceDef`/`Row.host`/`Author.build` existed | exported from `Api.lean:603`; no application, one test |
| `Transform.lean`, `Image.lean` | 151 | `Transform σ Γ A B E R` (a certified `Eff` in a wrapper), `ProgramImage` | exported from `Api.lean:605`; `Laws/Schema/{Transform,Image}` (four theorems); no application |
