...
Seventeen blocks, one region, three decision sites, an interrupt point before
...
The declared types the lowering emits are:
...
### 3.1 A flow performs one argument — `E4-TARGET-CE-022`
...
`Effect4Test/Counterexamples/Target/JobRequest.lean`. The forced repair is to
refuse the row where it is first misspelled — `familyTable` returning
`Option (List OpSpec)` — rather than emitting a module that does not
type-check. Lifting the restriction is a packet of its own.
...
* **Atoms are pure wire functions.** Confirmed, and unary (§3.1).
