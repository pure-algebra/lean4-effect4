...
## True at `ff0c042b` (2026-09-16, evening)

- Branch `refactor/phase1-phase3`. `make check` green: 313 modules, 54,229 declarations at `[propext, Quot.sound]`; the `Classical.choice` boundary is 2 modules, 29 declarations. The reader lane matches Lean's oracle on all 416 programs. The host lanes (`make check-host`) were last run by Codex before `1a97c37`.
...
1. Scope safety as a theorem: a successful `elaborate` yields a tree whose every level is in scope, proved per lift against `Node.binders`, so a wrong lift fails a proof rather than a program; then the agreement lemma between `Node.binders` and `effTy`'s environment extensions.
2. The rest of the authoring surface as generated data, so nothing is guessed: row wrappers from the row tables (native now, packages by DI-89), derived-form combinators from `Codegen/Forms.all` (one `Src` function per template, printed back by recognition; `catchTag` is already a template), and the checker's refusal projection (DI-86). Then the `eff { ... }` macro over `bindWith` and `andThen`, with the Lean binder as the source name (a macro, not a `Monad` instance: `Src` has no answer type).
3. The facade's additive half (DI-85), then S1, then the `Eff` series. The `select` step touches `branch` in three named places: `binders.json`, `Eff.suspendDecided`, `Straight.suspendDecided_iff`.
