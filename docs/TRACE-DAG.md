...
| coverage | `required-closed` | `generated/lowering-coverage.tsv` (twenty-five rules: eight straight-line `checked`, eight dispatch-form `covered`, `interrupt-point` `pinned` on the three M2 goldens, three region `checked`, four structured `covered`, `dispatch-fallback` `checked` on the irreducible program by the property loop `generated/lowering-property.tsv`: 200 generated flows, 1277 host runs, 276 frontiers, 318 sites both ways, 3/3 planted lowering mutants caught), `scripts/check-lowering-coverage.sh`, planted mutants (4/4) |
| interrupts | `required-open` (Lean face closed, 2026-09-03) | Packet M2. `Effect4/Flow/Interrupt.lean`: an `Interrupts` decision family over the region runner. Points are before every `perform` of a plain block and at every region `leave`; `Point.site` puts them at or above `interruptBase = 1000000`, disjoint from every `choose` site of an admitted flow (`Point.site_ne_choose`, `Point.site_inj`, decided per flow by `sitesSeparated`), so one wire carries both tapes. Exhaustion of the interrupt tape answers *not delivered* rather than a frontier, and a mask defers: `interruptPoint_masked_defers` keeps a delivered answer in `IState.pending`, `interruptPoint_unmasked_delivers` delivers it at the first unmasked point (rc.112's `_deferredInterrupt`). Delivery closes every open region innermost-first with `Outcome.interrupted` and ends the run `done interrupted` (`InterruptResult.interrupted`); `closeFrame_interrupted_log` is `closeFrame_log` at that outcome, so `Outcome.interrupted` finally has a Lean producer (A1 promised it; before M2 only the P-T11 projection could build one). `RunResult` is untouched and every region-runner theorem still speaks about `regionLoop`. Receipts: `Effect4Test/Flow/InterruptContract.lean` (the three goldens' logs, axioms `propext`/`Quot.sound`), rows `E4-FLOW-CE-022`, `E4-FLOW-CE-023`. Host evidence: `generated/traces/flow/interrupt/{interruptUnmasked,interruptMasked,interruptFinalizer}.tsv` agree with `harness/trace/interrupt-tail.ts` under every mask at both yield settings; the tail is the interruptor and calls `fiber.interruptUnsafe()` at the delivered site, the same rc.112 idiom `tracer.ts` uses on the budget path. **Open, exactly:** the mask is a model fact of the flow (`RegionProgram.masked`) that the lowering does not spell — no `Effect.uninterruptible` is emitted, and the tail withholds the interrupt itself — so rc.112's own deferral is not what the masked golden observes; and the interruptor identity is unwitnessable (the wire drops it, `frameExit .interrupted` carries `interrupt none`) |
