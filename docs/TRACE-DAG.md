...
| frame-simulation | `required-closed` **for the Lean pair only** | `Effect4.FrameSimulation.compile_simulates` (`Effect4/Semantics/FrameSimulation.lean`, packet D4, `test/contracts/frame-simulation.contract.md`): for `bound tape <= fuel` and `answersOf H p s0 = tape`, the frame machine started on `compile 0 p` finishes with `exitOf ((interpret H p).run.run s0).1`, with `H` into `ExceptT (Cause Val Unit Unit Unit) (StateT St Id)`; `compile_simulates_fail` is the `Cause.fail` specialisation. **Scope line, read it before quoting the row:** this is a simulation *modulo an effect oracle* — `PrimInterp` is pure and total, the answers are supplied by the tape, and `hOracle` is what makes that legitimate; what is proved is that the machine's control flow and exit equal the algebra's given agreeing answers. It covers the compiled fragment only (`success`, `failure`, `sync`, `onSuccess`; `inFragment` and `compile_inFragment` prove the rest is unreachable), says nothing about finalizers, brackets or regions (`Program` has no bracket former), and says nothing about the host: **the host agreeing with either emitter is still executable evidence under a mask, never a theorem.** Do not close this row globally |
...
| structured-agreement | `required-open` (dispatch half closed, flat structured half closed, 2026-09-03) | **The skeleton has a denotation.** `Effect4/Target/TypeScript/SkeletonSemantics.lean` (packet D3, axioms `propext`/`Quot.sound`, receipts in `Effect4Test/Target/TypeScript/SkeletonSemanticsContract.lean`) evaluates a lowered statement list into the *same* `Program (FullSig alphabet) (RunResult × Tape)` that `Effect4/Semantics/Denotation.lean` denotes a checked flow into, so the two forms are now compared as Lean terms and not as emitted bytes. One turn of a `dispatchLoop` is one block of the flow, so the skeleton's fuel is `Flow.denoteFuel`'s fuel unit for unit. *Proved*: **T3** `skeletonDispatch_denote` — for every checked flow and every tape, `⟦skeletonDispatch flow⟧ tape input = Flow.denote flow tape input`, with no side condition beyond admission; **T4 on the flat fragment** `skeletonStructured_denote_dispatch` — when the block graph has no merge node and no loop header (`Skel.Flat`, decided by `Skel.flatBelow` through `Skel.flat_of_flatBelow`) the structured form denotes the same `Program` as the dispatch form, through `skeletonStructured_denote`; and `Skel.execList_skeletonBlockWith`, the transfer-parametric block law both forms instantiate. *Open, exactly*: the other two shapes `Structuring.emitNode` emits. (a) A **merge node**: `emitNode` wraps its emission in `merge L<t>` and transfers to `t` by `break L<t>`; `Skel.afterBlock` does the right thing provided every `break L<t>` is dynamically inside its `label L<t>:`, which is `BreakScopedStatement` and needs `Structure.idom t` to lie on the `idom` chain of every predecessor of `t` (correctness of the Cooper–Harvey–Kennedy iteration in `Structure.idoms`). (b) A **loop header**: `emitNode` wraps it in `loop W<t>` and emits `continue W<t>` for a back edge; `Skel.loopRun` spends one unit of fuel per iteration, so the statement is a fuel statement of T3's shape, and it additionally needs that a back edge's target is the enclosing loop's header — a fact about `Structure.rpo` and `Structure.isBackEdge`. Neither is a gap in the denotation: `Skel.execList` interprets both shapes and the block law is stated over an arbitrary transfer; both owed facts are about the pinned `typescript` package's algorithms, not about Effect4's definitions, and are the same two `Effect4/Target/TypeScript/StructureLaws.lean` leaves open. Label well-scoping is unchanged and still proved there (`emitWith_wellScoped`). Host evidence unchanged: every flow golden through `structured-tail.ts`, the property corpus through `property-structured-tail.ts`, 1277 cases each way, a planted `continue`→`break` mutant caught |
...
| approximation | `required-open` (runner half closed, 2026-09-03) | `Effect4/Semantics/Approximation.lean` (DB-04 laws over the tape runner: fuel exhaustion is a live `frontier`, never a failure or refusal, and more fuel only extends a frontier run). The region-runner half (`regionLoop`/`runRegions`) was drafted against the pre-D2 failure carrier and did not survive D2's merged failure list; it is owed and is not in the tree. Nothing host-side is claimed |
