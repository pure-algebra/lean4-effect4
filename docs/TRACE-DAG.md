...
| structured-agreement | `required-open` | the structured form (`Effect4/Target/TypeScript/StructuredLower.lean`, `TypeScript/Structure.lean` v0.4.1) and the dispatch form are checked against the same Flow-runner goldens on the host (every flow golden through `structured-tail.ts`, the property corpus through `property-structured-tail.ts`, 1277 cases each way, a planted `continue`→`break` mutant caught); the Lean theorem that they agree on every flow is owed |
