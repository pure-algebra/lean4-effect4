...
Status: **EXECUTED THROUGH S4, 2026-09-02.** S5 landed 2026-09-02 in lean4-whatwg; S6 (composition extensions) remains open by design. This is the living
...
| S5 | `lean4-whatwg` `bd5b405` (slice W4 of its `docs/WHATWG-PACKAGE-PLAN.md`) | `[[require]] effects` at `5611c3a`; probe: exact pin, Apache-2.0 at the tag, zero transitive packages, same toolchain, `lake build effects/Effects` green; own build, trust self-test, vendor seal green | S5 exit met. The streams repository is now the `whatwg` package (renamed 2026-09-02) and also requires `hash`; nothing imports `effects` until its P4 |
