...
`Bridge.lean` and `ocaml/link`), are archived apart from `Bridge.lean` itself (see "What is
not here"). Everything under
...
Lean half, `src/OCaml5/Bridge.lean`, remains only because `harness/truth/Truth.lean` cites
it and is a stamp input of the truth family; it leaves with the next edit that re-stamps the
truth artefacts.
