...
| `tools/ProofGraph` | shared tooling evidence: exact theorem references, speculative tactic search, checked proof publication, obligation joins and dependency checks; no Effect4 or Aesop dependency; both Laws and Conform consume it |
| `tools/Conform` | generic source descriptions, exact conformance reports, proof bindings through `ProofGraph`, layouts, model builders, specification generation and compiler inspection; `Conform.Effect4` owns project profiles and target fixtures; `scripts/check-conform.sh` owns fresh execution receipts |
