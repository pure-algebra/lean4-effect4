...
Passive leaves and the recursive field-admission judgment are closed. The
mutual representation/check family remains `required-open` only because the
broader general-recursor half of `SC-REP-03` is still open. Document meaning,
...
keeps `SCHEMA-PG-PAYLOAD/SC-REP-03-RECURSOR` visibly `required-open`.
