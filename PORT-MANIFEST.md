...
Standard Schema V1, and JSON Patch/XML. A `grep -ic` for each returned zero in
both documents when the survey was written. It no longer does: `Optic` is
non-zero in both, and the other five are non-zero in this file. Every one of
those hits is this paragraph, the `Bottom` type-parameter note below, or
Effect4's own `Effect4.Data.Optic` carrier and its `DATA-PG-OPTIC` graph. None
is a disposition row for the Effect face of that name.
...
| --- | ---: | --- | --- | --- |
| `library/cas/Cas/Lang/Sig.lean` | 1-27 | `91e9d984fead2a36c4503bb8db979e8f085123db754a655846888aac41e87cd6` | `Effects/Algebra/Signature.lean` in lean4-effects `v0.1.0` (`5611c3a`) | `owned`; operation and answer universes generalized |
| `library/cas/Cas/Lang/Prog.lean` | 1-54 | `a8ac15632d155f9558db1969f2a25faf548e337c35b584f54316d5aba0f958aa` | `Effects/Algebra/Program.lean` in lean4-effects `v0.1.0` (`5611c3a`) | `owned`; well-founded higher-order proof carrier only |
| `library/cas/Cas/Lang/Handler.lean` | 39-67 | `7170bd9c6de8743d10fff712644fa1d6ce59100e5dc33e0458b17897de933bf9` | `Effects/Algebra/Handler.lean` in lean4-effects `v0.1.0` (`5611c3a`) | `split`; generic handler moved, CAS reference handler retained |
| `library/cas/Cas/Lang/Representation.lean` | 35-118 | `5ca0f4cfeeb396c0f084d547a72825ddbe8a6c6a37e0786dbed9e94d68b652a8` | `Effects/Algebra/Laws.lean` in lean4-effects `v0.1.0` (`5611c3a`) | `split`; generic laws moved, CAS observation retained |
...
| `library/cas/Cas/Lang/Tower.lean` | 63-85 | `02b348347431dd80544427bb93fd0c0878b37ac010196ee1faf0c5925773027a` | `Effects/Algebra/Handler/Composition.lean` in lean4-effects `v0.1.0` (`5611c3a`) | `split`; generic composition moved, byte plane retained |
| `library/cas/Cas/Backend/SumAlgebra.lean` | 172-173, 196-467 | `d6a67f6efa75d81662f79945ce84ce54e83392b786bd9ce5a4884c01c2f7070b` | `Effects/Algebra/Sum.lean` in lean4-effects `v0.1.0` (`5611c3a`) | `split`; generic sum laws moved |
| `library/cas/Cas/Backend/Universal.lean` | 127-223 | `cce4f2a50a7e2a2e8b57b4de1a82f90c86efcb9749683795fd2b9e96d2ad8010` | `Effects/Algebra/MonadLaws.lean` in lean4-effects `v0.1.0` (`5611c3a`) | `split`; minimal equation bundles moved |
| `library/cas/Cas/Backend/Universal.lean` | 294-349, 469-625 | same file digest | `Effects/Algebra/Universal.lean` in lean4-effects `v0.1.0` (`5611c3a`) | `split`; freeness and model initiality moved |
| `library/cas/Cas/Backend/Universal.lean` | 739-749, 757-768, 785-790 | same file digest | `Effects/Algebra/Handler/Category.lean` in lean4-effects `v0.1.0` (`5611c3a`) | `split`; category laws and endomorphism monoid moved |
...
`Effect4/Data/Row.lean` now owns the only checked row and canonicality
predicate. The fixed battery and `generated/data-row-assurance.tsv` join the
exact 60-name module census, separate 34-name authored API, 34 standard-order
hypothesis profiles, 23 theorem and axiom receipts, nine counterexamples,
eight duplicate-name exclusions, all ten `DATA-PG-ROW` edges, and five
detector reactions. No `Classical.choice` enters the theorem spine.

| Stable type row | Exact Lean declaration and owner | Canonical role | Duplicate prevention | Assurance |
| --- | --- | --- | --- | --- |
| `E4-TYPE-DATA-ROW` | `Effect4.Row`; `Effect4.Data.Row` | sole proof-carrying finite canonical row; raw admission is `Row.normalize : List α -> Row α` | no `RawRow`, `CheckedRow`, `CanonicalRow`, comparator carrier, unchecked `ofList`, or noncanonical append route | `proofGraphId = DATA-PG-ROW`; `closureSource = generated/data-row-assurance.tsv`; generated closed |
| `E4-TYPE-DATA-ASCENDING` | `Effect4.Ascending`; `Effect4.Data.Row` | sole strict-list canonicality predicate, using the ambient Std order classes | no area-local, comparator-relative, or `Nodup`-only replacement | attached to `DATA-PG-ROW/IDENTITY`; `closureSource = generated/data-row-assurance.tsv`; generated closed |
...
type-code carrier. The generated owned-declaration/API/owner/theorem/axiom join
and its five mutation reactions are green. The authored rows below allocate
routes and point at the generated closure source; they carry no manual
`required-closed` override. The generated projection closes the three passive
identity rows through `ENV-LEAF-KEY-IDENTITY`. The generated projection also
closes the local `ENV-LEAF-KEY-ORDER-BRIDGE` receipt after exact instance,
relation, computation, and axiom checks; its `DATA-PG-ROW/ORDER` attachment
is a delegated route whose status is owned by the closed Data.Row assurance
join. `ServiceUniverse` remains `required-open` only at the existing
`ENV-KEY-INTERP` semantic edge of the shared Context graph.
...
| --- | --- | --- | --- | --- | --- |
| `E4-TYPE-ENV-SERVICE-NAME` | `Effect4.ServiceName`; `Effect4.Context.Key` | sole native nominal service identity; first-order `Nat` wrapper; no wire spelling | native contract `test/contracts/environment-context-key.contract.md`; `owned`; no Foldlab declaration ported | distinct from `ServiceTypeCode`; a later Effect string tag is a target/profile spelling, not another native identity | `leafReceiptId = ENV-LEAF-KEY-IDENTITY`; `receiptId = ENV-AX-KEY`; `evidenceId = ENV-EV-KEY-CONTRACT`; `closureSource = generated/environment-context-key-assurance.tsv`; `status = generated` |
| `E4-TYPE-ENV-SERVICE-TYPE-CODE` | `Effect4.ServiceTypeCode`; `Effect4.Context.Key` | sole native first-order code selecting a service carrier relative to a supplied universe | same native contract and disposition | distinct from `ServiceName`; neither a Lean `Type`, an inverse of one, nor a second operation/answer alphabet | same key-identity leaf; `closureSource = generated/environment-context-key-assurance.tsv`; `status = generated` |
| `E4-TYPE-ENV-SERVICE-KEY` | `Effect4.ServiceKey`; `Effect4.Context.Key` | sole native context-key identity, the ordered pair `(name, service)` with the frozen name-major order; the Std `LE`, `Std.IsLinearOrder`, and `Std.LawfulOrderLT` bridge is derived from that same `<` | same native contract and disposition | not type-indexed and not Effect's later string-only `Context.Tag` target view; no second Context or Requirement key carrier, comparison relation, or order-law carrier | same key-identity leaf; `parentGraphEdge = ENV-PG-CONTEXT/identity`; `orderBridgeLeafReceiptId = ENV-LEAF-KEY-ORDER-BRIDGE`; `orderBridgeParentEdge = DATA-PG-ROW/ORDER`; `orderBridgeClosureSource = generated/environment-context-key-assurance.tsv`; `orderBridgeStatus = generated`; `closureSource = generated/environment-context-key-assurance.tsv`; `status = generated` |
| `E4-TYPE-ENV-SERVICE-UNIVERSE` | `Effect4.ServiceUniverse`; `Effect4.Context.Key` | supplied semantic boundary reading codes as Lean types; never canonical content, persisted data, or a source of code identity | same native contract; `foreignBoundary`; no Foldlab declaration ported | one shared Context interpretation boundary; no universe field on `ServiceKey`, no inverse `Type -> ServiceTypeCode`, and no per-service duplicate universe | `proofGraphId = ENV-PG-CONTEXT`; `nodeId = ENV-KEY-INTERP`; `receiptId = ENV-AX-KEY`; `declarationJoinSource = generated/environment-context-key-assurance.tsv`; `declarationJoinStatus = generated`; `required-open` |

The additive Std bridge's authored public API is exactly 14 declarations:
...
`ServiceKey.instLawfulOrderLT`. The 97-name owned-declaration census separately
...
| --- | --- | --- | --- |
| `E4-TYPE-SCHEMA-FLOAT64` | `Effect4.Float64`; `Effect4.Data.Json` | nominal 64-bit IEEE payload datum; not Lean `Float`, host equality, or a five-value classification | `leafReceiptId = SCHEMA-LEAF-FLOAT64-BITS`; parent `SCHEMA-PG-PAYLOAD`; `declarationJoinSource = generated/schema-structural-assurance.tsv`; `required-closed` |
| `E4-TYPE-SCHEMA-JSON` | `Effect4.Json`; `Effect4.Data.Json` | recursive JSON payload with ordered object-entry lists so duplicate keys remain representable; not map-like `Lean.Json` | `nodeId = SCHEMA-NODE-JSON-FINITENESS`; parent `SCHEMA-PG-PAYLOAD`; `declarationJoinSource = generated/schema-structural-assurance.tsv`; `required-closed` |
| `E4-TYPE-SCHEMA-REFERENCE-KEY` | `Effect4.ReferenceKey`; `Effect4.Schema.Payload` | raw reference spelling; non-emptiness is admission, not hidden construction | `leafReceiptId = SCHEMA-LEAF-PAYLOAD-SCALARS`; parent `SCHEMA-PG-PAYLOAD`; `declarationJoinSource = generated/schema-structural-assurance.tsv`; `required-closed` |
| `E4-TYPE-SCHEMA-GLOBAL-SYMBOL-KEY` | `Effect4.GlobalSymbolKey`; `Effect4.Schema.Payload` | portable global-symbol identity only; local symbols have no constructor | same scalar leaf receipt; `declarationJoinSource = generated/schema-structural-assurance.tsv`; `required-closed` |
| `E4-TYPE-SCHEMA-ANNOTATION-ENTRY` | `Effect4.AnnotationEntry`; `Effect4.Schema.Payload` | one retained JSON annotation entry; encode-side pruning remains a wire policy | same scalar leaf receipt; `declarationJoinSource = generated/schema-structural-assurance.tsv`; `required-closed` |
| `E4-TYPE-SCHEMA-ANNOTATIONS` | `Effect4.Annotations`; `Effect4.Schema.Payload` | exact alias `Option (List AnnotationEntry)` preserving absent versus present-empty | same scalar leaf receipt; `declarationJoinSource = generated/schema-structural-assurance.tsv`; `required-closed` |
| `E4-TYPE-SCHEMA-LITERAL-VALUE` | `Effect4.LiteralValue`; `Effect4.Schema.Payload` | four-constructor decoded literal value; no null or symbol constructor | same scalar leaf receipt plus constructor-cap receipt; `declarationJoinSource = generated/schema-structural-assurance.tsv`; `required-closed` |
| `E4-TYPE-SCHEMA-ENUM-VALUE` | `Effect4.EnumValue`; `Effect4.Schema.Payload` | two-constructor enum value; `EnumValue.toLiteralValue : EnumValue -> LiteralValue` is the sole total injective raw embedding and preserves every binary64 payload, while literal finiteness is owned separately by `SCHEMA-PG-FIELD-ADMISSION` | same scalar leaf receipt plus `SCHEMA-REL-ENUM-TO-LITERAL-VALUE`; `declarationJoinSource = generated/schema-structural-assurance.tsv`; `required-closed` |
| `E4-TYPE-SCHEMA-ENUM-ENTRY` | `Effect4.EnumEntry`; `Effect4.Schema.Payload` | enum name/value pair; duplicate names and aliases remain representable | same scalar leaf receipt; `declarationJoinSource = generated/schema-structural-assurance.tsv`; `required-closed` |
| `E4-TYPE-SCHEMA-PROPERTY-KEY` | `Effect4.PropertyKey`; `Effect4.Schema.Payload` | string, number, or global-symbol portable key; no local-symbol constructor | same scalar leaf receipt plus constructor-cap receipt; `declarationJoinSource = generated/schema-structural-assurance.tsv`; `required-closed` |
| `E4-TYPE-SCHEMA-REPRESENTATION-ANNOTATION` | `Effect4.RepresentationAnnotation`; `Effect4.Schema.Payload` | required `id`/`payload` record; intentionally has no `schemas` field | `leafReceiptId = SCHEMA-LEAF-PAYLOAD-RECORDS`; parent `SCHEMA-PG-PAYLOAD`; `declarationJoinSource = generated/schema-structural-assurance.tsv`; `required-closed` |
| `E4-TYPE-SCHEMA-CHECK-REPRESENTATION-ANNOTATION` | `Effect4.CheckRepresentationAnnotationOf`; `Effect4.Schema.Payload` | parameterized check annotation with optional referenced schemas; not merged with `RepresentationAnnotation` | same record leaf receipt; `declarationJoinSource = generated/schema-structural-assurance.tsv`; `required-closed` |
| `E4-TYPE-SCHEMA-ELEMENT` | `Effect4.ElementOf`; `Effect4.Schema.Payload` | parameterized array-element record; applied alias `Element` adds no carrier | same record leaf receipt; `declarationJoinSource = generated/schema-structural-assurance.tsv`; `required-closed` |
| `E4-TYPE-SCHEMA-PROPERTY-SIGNATURE` | `Effect4.PropertySignatureOf`; `Effect4.Schema.Payload` | parameterized property record; applied alias adds no carrier | same record leaf receipt; `declarationJoinSource = generated/schema-structural-assurance.tsv`; `required-closed` |
...
| `E4-JUDGMENT-SCHEMA-TYPESCRIPT-GENERATION-READY` | `Target.TypeScript.Schema.GenerationReady`; `Effect4.Target.TypeScript.Schema` | checked bridge predicate over existing `Document`, `Json`, and TypeScript syntax; rejects target-name collisions and object-entry collapse | `proofGraphId = TS-PG-SCHEMA-DOCUMENT-GENERATION`; target host and raw 22-representation/2-check coverage edges closed by `check-schema-typescript-generation.sh`; raw `Json` lowering has a proved left inverse and injectivity; document revival / decoded-value denotational simulation remains open |
| `E4-BRIDGE-SCHEMA-TYPESCRIPT-DOCUMENT` | `json`, `representation`, `documentExpr`, `module?`, `generate?`; `Effect4.Target.TypeScript.Schema` | recursive `derivedExpansion` into existing target syntax and `targetOnly` source bytes; no generated-schema carrier | same graph; exact fixture and complete raw-tag digest, direct TypeScript, pinned Effect revival, language-service diagnostics, and central `E4-TARGET-CE-004` receipt |
...
The closure authority is the generated projection, not the status text in any
frozen breaker. `./scripts/check-schema-structural-assurance.sh` regenerates
and byte-compares the 1,298-name owned-declaration census, 493 theorem and
axiom receipts, 32 counterexamples, nine duplicate-prevention checks, exact
...
The table records the frozen initial allocation. The generated structural
join now closes all six initially open edges; the four `not-applicable` edges
remain unchanged. A later semantic, bridge, or target claim must first change
the corresponding `not-applicable` row through a new breaker packet.
...
field-admission breaker remains Pass-B frozen, while its implementation,
production ownership gate, proof receipts, and generated join are now green.
This closes the structural rows recorded above, not Schema denotation, wire,
document, codec, or host cutover.

The exact rc.112 bytes are vendored locally at
`vendor/effect-4.0.0-rc.112/src/SchemaRepresentation.ts`, at SHA-256
...
| Narrow algebra gate | in lean4-effects `v0.1.0` (`5611c3a`): `lake env lean EffectsTest/Algebra/ExtractionContract.lean` | frozen public algebra signatures and the `E4-ALG-CE-*` counterexamples |
| Narrow retained-closure gate | in lean4-effects `v0.1.0` (`5611c3a`): `lake env lean EffectsTest/Algebra/RetainedClosureContract.lean` | exact retained Foldlab declaration signatures, binder order, universes, and their closure theorems |
| Narrow flow admission gate | in lean4-effects v0.4.0 (`e86f141`): `lake env lean EffectsTest/Flow/FlowV2Contract.lean` and `lake env lean EffectsTest/Counterexamples/Flow/FlowV2.lean` | frozen Flow v2 declarations plus the retained `E4-FLOW-CE-001`-`004`, `007`, `013`, `014`, `016` witnesses and `EF-FLOW-CE-001`-`003`; `E4-FLOW-CE-005` is superseded by `EF-FLOW-CE-002` |
| Narrow flow privacy gate | in lean4-effects v0.4.0: the retained `E4-FLOW-CE-015` section of `EffectsTest/Flow/FlowV2Contract.lean` | `E4-FLOW-CE-015`; both unchecked `CheckedFlow` construction paths must stay unresolvable from an importing module |
...
| Schema payload declaration-surface and import-ownership gate | `./scripts/check-schema-payload-surface.sh`; `./scripts/test-schema-payload-surface-gate.sh` | the elaborated surface checker covers the exact carrier/alias/type-valued-definition surface and 67 constructor/field entries. Its reaction test kills four shape mutations, an unallocated alias, an ordinary public type-valued definition, four D7 owner drifts, a declaration-free upward import, and two source overrides: 13/13. The production gate proves D0-D3 reachability and D4-D7 non-leakage at the Payload-only boundary and exact owners along `Check -> Document -> Representation -> Payload -> Data.Json`. This is a finite declaration/ownership receipt consumed by the generated structural join, not a separate proof graph or semantic claim |
| Schema structural generated assurance gate | `./scripts/check-schema-structural-assurance.sh`; `./scripts/test-schema-structural-assurance-gate.sh` | regenerates and byte-compares `generated/schema-structural-assurance.tsv`; joins all 1,298 declarations owned by seven structural/annotation source modules, all 493 theorem and kernel-dependency receipts, 32 registered counterexamples, nine duplicate-prevention names, exact rc.112 source pins, repaired surface evidence, optic and annotation laws, and every allocated structural graph edge. Three detector reactions reject stale output, manual closure, and source override. The tag, payload, field-admission, optic, and annotation-data graphs close all applicable edges. |
| Environment Context Key generated-evidence gate | `./scripts/check-environment-context-key-evidence.sh` | regenerates and byte-compares `generated/environment-context-key-assurance.tsv`; joins the exact 97-name module-owned census (including compiler-generated companions), a separate 25-name frozen Context Key API, a separate 14-name Std bridge API, four type allocations, four contract obligations, six registered counterexample IDs, eight original and seven bridge theorem receipts, 37 axiom receipts, eight exact bridge synthesis/relation/computation receipts, `ENV-EV-KEY-CONTRACT`, `ENV-LEAF-KEY-IDENTITY`, and `ENV-LEAF-KEY-ORDER-BRIDGE`. The projection closes the passive key-family and local bridge receipts; it emits only a delegated route to `DATA-PG-ROW/ORDER`, whose status belongs to Data.Row. `ServiceUniverse` remains the sole semantic Context attachment at open `ENV-PG-CONTEXT/ENV-KEY-INTERP` |
| Environment Context Key evidence reaction test | `./scripts/test-environment-context-key-evidence-gate.sh` | rejects one extra module-owned declaration, one missing declaration, one declaration owner drift, one stale generated projection, and one attempted manual `required-closed` override through the same detector. This is bounded detector evidence, not a proof graph or a Context semantic claim |
| Data.Row generated assurance gate | `./scripts/check-data-row-assurance.sh` | regenerates and byte-compares `generated/data-row-assurance.tsv`; checks the 60-name owned census, separate 34-name API and exact hypothesis profiles, 23 theorem/axiom receipts, eight forbidden duplicate routes, nine registered counterexamples, the delegated Context order-bridge dependency, and all ten `DATA-PG-ROW` edges |
| Data.Row assurance reaction test | `./scripts/test-data-row-assurance-gate.sh` | rejects extra, missing, owner-drifted, stale-output, and authored-manual-closure mutants through the same detector; 5/5 is required |
...
| Human-readable axiom receipts | `lake env lean EffectsTest/Algebra/AxiomReport.lean` in lean4-effects `v0.1.0` (`5611c3a`); `lake env lean EffectsTest/Flow/FlowV2AxiomReport.lean` in lean4-effects `v0.4.0` (`e86f141`); `lake env lean Effect4Test/Schema/AxiomReport.lean`; `lake env lean Effect4Test/Data/AxiomReport.lean`; `lake env lean Effect4Test/Environment/AxiomReport.lean` | every currently exported algebra, flow, Schema, Data.Row, and Context Key theorem named by the current receipt sets, with its axiom dependencies printed |
...
| Internal citation gate | `./scripts/check-internal-citations.sh` | rejects every `<doc>:<line>` or `<doc>:<line>-<line>` citation naming `docs/SCHEMA-CUTOVER.md`, `PLAN.md`, `AGENTS.md`, `docs/ARCHITECTURE.md`, `PORT-MANIFEST.md`, or `docs/AGENT-ROUTING.md` anywhere under `Effect4/`, `Effect4Test/`, `docs/`, `test/`, `scripts/`, or `harness/`, because those documents are authored and edited continuously so a line number silently retargets; line citations into the pinned host sources, into `vendor/foldlab/`, and into `.lean` sources are examined and deliberately accepted; a lexical scan only, so it resolves no anchor, does not check that a cited section still says what the citing sentence claims, and assigns no cutover status |
