...
and byte-compares the 1,092-name owned-declaration census, 418 theorem and
axiom receipts, 26 counterexamples, nine duplicate-prevention checks, exact
...
| Schema structural generated assurance gate | `./scripts/check-schema-structural-assurance.sh`; `./scripts/test-schema-structural-assurance-gate.sh` | regenerates and byte-compares `generated/schema-structural-assurance.tsv`; joins all 1,092 declarations owned by the five structural source modules, all 418 theorem and kernel-dependency receipts, 26 registered counterexamples, nine duplicate-prevention names, exact rc.112 source pins, repaired surface evidence, nine leaf receipts, and every allocated structural graph edge. Three detector reactions reject stale output, manual closure, and source override. The tag and field-admission graphs close; the payload graph retains only its named general-recursor edge open |
