...
remains open. `ServiceUniverse` remains `required-open` only at the existing
...
| Environment Context Key generated-evidence gate | `./scripts/check-environment-context-key-evidence.sh` | regenerates and byte-compares `generated/environment-context-key-assurance.tsv`; joins the exact 97-name module-owned census (including compiler-generated companions), a separate 25-name frozen Context Key API, a separate 14-name Std bridge API, four type allocations, four contract obligations, six registered counterexample IDs, eight original and seven bridge theorem receipts, 37 axiom receipts, eight exact bridge synthesis/relation/computation receipts, `ENV-EV-KEY-CONTRACT`, `ENV-LEAF-KEY-IDENTITY`, and `ENV-LEAF-KEY-ORDER-BRIDGE`. The authored manifest supplies allocations and generated-source pointers, not closure. The projection closes the passive key-family leaf and local bridge receipts only; `ServiceUniverse` remains the sole semantic Context attachment at open edge `ENV-PG-CONTEXT/ENV-KEY-INTERP`, while the bridge attaches to open `DATA-PG-ROW/ORDER` |
...
| Human-readable axiom receipts | `lake env lean Effect4Test/Algebra/AxiomReport.lean`; `lake env lean Effect4Test/Flow/AxiomReport.lean`; `lake env lean Effect4Test/Schema/AxiomReport.lean`; `lake env lean Effect4Test/Environment/AxiomReport.lean` | every currently exported algebra, flow, Schema, and Context Key theorem named by the current receipt sets, with its axiom dependencies printed |
