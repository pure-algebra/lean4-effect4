...
| `E4-JUDGMENT-SCHEMA-TYPESCRIPT-GENERATION-READY` | `Target.TypeScript.Schema.GenerationReady`; `Effect4.Target.TypeScript.Schema` | checked bridge predicate over existing `Document`, `Json`, and TypeScript syntax; rejects target-name collisions and object-entry collapse | `proofGraphId = TS-PG-SCHEMA-DOCUMENT-GENERATION`; target host and raw 22-representation/2-check coverage edges closed by `check-schema-typescript-generation.sh`; denotational simulation open |
