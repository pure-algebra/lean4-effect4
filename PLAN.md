...
4. **In progress.** One Context/Service/Layer/Scope/ManagedRuntime
   ...
   `ServiceUniverse` as its only type-reading boundary. Its leaf closure waits
   on the generated declaration join. Its sibling L0 node `Data/Row` is open
   and blocks all of L1.
...
| 2 | `Effect4.ServiceName`, `ServiceTypeCode`, `ServiceKey`, and `ServiceUniverse` | one local key-family leaf receipt, with `ENV-KEY-INTERP` attached to the Context graph | The carrier signatures, exact name-major order, collision relation, universe-relative carrier, explicit transport, carrier-collision witness, and axiom receipt are implemented and green. The four public-type rows are required, and closure waits on the generated declaration join. Before `Data.Row` freezes, probe a bridge to Lean 4.33's standard lawful-order package. Adopt that bridge if it preserves the frozen relation; do not mint an Effect4-specific order-law carrier. |
