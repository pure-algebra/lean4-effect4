/** The Lean-owned Forms.Template fold for the sequencing and handler forms.
...
import type { Lit } from "../eff.gen.ts"
...
export interface FormArguments<E, T> {
  readonly effects: readonly EffectSlot<E>[]
  readonly terms?: readonly T[]
}
export interface FormAlgebra<E, T> {
  literal(value: Lit): T
...
/** Unsupported template constructors refuse explicitly; the remaining foreign
 * forms keep their existing adapters until their lowering is integrated here. */
export function expandTemplate<E, T>(template: Template, base: number,
    args: FormArguments<E, T>, algebra: FormAlgebra<E, T>): Expansion<E> {
  try {
    natural(base)
    ...
    const walk = (t: Template, depth: number): E => {
      switch (t._tag) {
        ...
        case "die": case "service": case "yieldNow": case "fork": case "forkIn":
        case "forkScoped": case "acquireRelease": return invalid(`unsupported template ${t._tag}`)
      }
    }
...
export function expandForm<E, T>(name: string, base: number,
    args: FormArguments<E, T>, algebra: FormAlgebra<E, T>): Expansion<E> {
