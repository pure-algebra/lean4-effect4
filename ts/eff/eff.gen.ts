...
export type LayerTerm =
  ...
  | { readonly _tag: "ref"; readonly target: ReadonlyArray<number> }
  | { readonly _tag: "mergeAll"; readonly layers: ReadonlyArray<LayerTerm> }
...
export const LayerTerm = Schema.TaggedUnion({
  ...
  ref: { target: Schema.Array(Schema.Int) },
  mergeAll: { layers: Schema.Array(Schema.suspend((): Schema.Codec<LayerTerm> => LayerTerm)) },
})

// LayerTerms: the cons-list of LayerTerm, carried as ReadonlyArray<LayerTerm>.

export const RowKind = Schema.Literals(["sync", "async", "program"])
export type RowKind = typeof RowKind.Type

export const RowShape = Schema.Literals(["call", "value", "tupleCall", "method"])
...
export const EffTy = Schema.Struct({
  answer: Schema.suspend((): Schema.Codec<Ty> => Ty),
  error: Schema.suspend((): Schema.Codec<Ty> => Ty),
  requires: Schema.Array(ServiceKey).check(Schema.makeFilter((xs) => xs.every((b, i) => i === 0 || ((a) => a !== undefined && (a.name.value < b.name.value || (a.name.value === b.name.value && a.service.value < b.service.value)))(xs[i - 1])))),
})
