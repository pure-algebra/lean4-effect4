...
export const layerTermJson = (v: LayerTerm): Json => {
  switch (v._tag) {
    ...
    case "ref": return ["ref", v.target.map((y) => y)]
    case "mergeAll": return ["mergeAll", layerTermsJson(v.layers)]
  }
}

export const layerTermsJson = (v: ReadonlyArray<LayerTerm>): Json => consList(v, (y) => layerTermJson(y))
...
export const rowJson = (v: Row): Json => ({
  ...
  registration: registrationJson(v.registration),
})

export const effTyJson = (v: EffTy): Json => ({
  answer: tyJson(v.answer),
  error: tyJson(v.error),
  requires: v.requires.map(serviceKeyJson),
})
