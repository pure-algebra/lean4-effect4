...
**What it is.** A *syntax-level* transpiler from the `Prim`-free function group of
`src/Effect4/Machine/Fibers.lean` to `OCaml5.Ml.Syntax`: `collect` parses a Lean file with Lean's
own parser (no elaboration, no lake), `trDef` translates one `def`'s surface syntax to an
`Ml.Bind` (a `where`-local is addressed as `outer.local`), applies the pure-update → mutation
pass (`Ml.Passes.mutate`) to the names declared linear, and records every syntax form it does not
know as `Expr.hole "<kind>"`, so the residue is visible in the output and countable. Packet 2 of
`docs/research/2026-09-04-seat-f3-ocaml-platform-critique.md` §6; `Tools/TranspileDeep.lean`
is the driver.
...
avatar constructor), the method table (`List.*`/`Option.*` dot-calls → `Stdlib`), the renames
(`fiber?` → `fiber_opt`), the anonymous-constructor hints (which record `⟨a, b⟩` builds, per def),
and the linear names per def (the mutate pass's precondition, `Ml/Passes.lean`). DIVERGENCE 3 is
one rewrite: `m.update x` and `m.modify …` read as their receiver, because the avatar's records
are mutated in place.

**Depends on.** `OCaml5.Ml.Passes` (and through it `Ml.Syntax`, `Ml.Render`, `Ml.Identifier`),
`Lean.Parser`.

**Properties.**
* **Every unknown form is a hole**, never a silent drop — *by construction* (`hole` records it in
  the residue; `--residue` prints the count per def).
* **Scoping is syntactic**: a name is local iff a binder in scope introduced it; everything else
  goes through the tables or `globalName` — *by construction*.
* **The tables are data**, listed once at the top of this file — *by construction*.
-/
