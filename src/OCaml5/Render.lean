...
`src/OCaml5/Ml/`, shaped like the estate's TypeScript target package
...
data about `workshop/Deep/Fibers.lean` and about `ocaml/avatar/deep_fibers.ml` rather
...
`ocaml/avatar/deep_fibers.ml` transcribed `workshop/Deep/Fibers.lean` by hand. These
...
#guard Avatar.runEvent.ctors.length == 21
#guard Avatar.runDecision.ctors.length == 7
#guard Avatar.cmd.ctors.length == 8
#guard Avatar.withFiberAction.ctors.length == 20
-- The three `Cmd` arms with no OCaml existence say so; the five others do not.
#guard (Avatar.cmd.ctors.filter (·.comment.isSome)).map (·.leanName) == ["loop", "deliver", "finish"]
#guard Avatar.cmd.ctors.map (CtorDesc.ocaml "C") ==
...
Rendered by `src/OCaml5/Tools/RenderDeep.lean` (a `lake env lean --run` driver that lives
...
/-! ## `Effect4/Deep/Stores.lean` → `ocaml/avatar/deep_stores.ml` -/
...
/-! ## `Effect4/Deep/Layer.lean` → `ocaml/avatar/deep_layer.ml` (seat F2)
...
/-! ## `Effect4/Deep/Context.lean` → `ocaml/avatar/deep_context.ml` (seat F2)
...
/-! ## `Effect4/Deep/ForkFlow.lean` → `ocaml/avatar/deep_forkflow.ml` (seat F2)
...
#guard (Avatar.withFiberAction.ctors.map (·.leanName)).all
  (fun n => (Deep.Stores.actionName.ctors.map (·.leanName)).contains n || n == "awaitAllFailFast")
