...
out of the environment and prints the description in the same `where` syntax `Render.lean`
uses, so the two can be diffed before the hand copies are retired.
...
/-- `--module Ns a=Full.Name b=Full.Name …`: a whole generated module. -/
