...
`Machine.cell`, and `emit`, which prints exactly the row `git:14e6835:ocaml/tools/run-witness.sh` read. -/
...
— is an executed one; `git:14e6835:ocaml/tools/fuzz.sh --witnesses` ran it on all thirteen and the
