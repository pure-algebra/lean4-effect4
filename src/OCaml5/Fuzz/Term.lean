...
**What it is.** The random `Term` generator of spike P5 over the effect fragment, its filter
(`classify`: admissible, finishes, prints rows a host can print), its shrinker (`shrinks`,
`stepsOf`), and the two minimised host-vs-host disagreements the 1,300-program campaign found,
pinned with their four row lists. `OCaml5.Fuzz` (the driver) and `OCaml5.Compile.Fuzz` draw from
it. Report: `docs/research/2026-09-03-spike-p5-fuzz.md`.

**Depends on.** `OCaml5.Fuzz.Gen`, `OCaml5.Render` (`Term.render`, and through it `Effect`,
`Compiler` and `Witnesses`).

**Properties.**
* **Well-sorted and admissible by construction**, closed and terminating (no recursion in the
  grammar) — see below; *tested* on sixty seeds (the `#guard`s at the end).
* **Shrink paths replay**: `stepsOf seed size path` regenerates a pinned term — *tested*.
