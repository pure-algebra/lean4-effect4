...
are proved of it and *projected onto* `Effect4.Machine.Dispatcher`, which is the object the Deep
machine actually runs (`src/Effect4/Machine/Fibers.lean:114-152`, transcribing `Scheduler.ts:105-131`
and `:225-233`).
...
* `Dispatcher.bucketPairs_enqueue` — `Effect4.Machine.Dispatcher.enqueue` *is* `Buckets.enqueue`
  through `bucketPairs`.
...
* `Dispatcher.drain_projection` — `Effect4.Machine.Dispatcher.drain` *is* `Buckets.drain`.
...
| `Buckets` | the avatar's priority buckets; `Effect4.Machine.Dispatcher.buckets` | the projection |
| `Buckets.enqueue` | `Effect4.Machine.Dispatcher.enqueue` (`Scheduler.ts:105-131`) | `bucketPairs_enqueue` |
| `Buckets.drain` | `Effect4.Machine.Dispatcher.drain` (`runTasks`, `Scheduler.ts:225-233`) | `drain_projection` |
...
* **`armed`.** `Effect4.Machine.Dispatcher` carries `armed : Bool` — "the host callback is
...
* **The cross-dispatcher flush order.** `src/Effect4/Machine/Fibers.lean:1130` records it as an
...
/-- `Effect4.Machine.Dispatcher.enqueue` (`Scheduler.ts:105-131`): append at the tail of the
...
/-- `Effect4.Machine.Dispatcher.drain` (`runTasks`, `Scheduler.ts:225-233`): the snapshot, and an
empty dispatcher. -/
...
ascending priorities. `Effect4.Machine.Dispatcher.insert` is `private`, so the invariant is
recovered here from the *reflected* unfolding equation `enqueue_unfold`, which holds by `rfl`. -/
def Wf (d : Effect4.Machine.Dispatcher ν σ β ε δ ι α) : Prop :=
  strictAsc (bucketPairs d.buckets) = true

/-- The unfolding of `Effect4.Machine.Dispatcher.enqueue` at a non-empty bucket list, which holds
...
theorem enqueue_unfold (a : Bool) (p : Nat) (t : Task ν σ β ε δ ι α)
    ...
    ((Effect4.Machine.Dispatcher.mk (b :: rest) a).enqueue p t).buckets
      = (if b.priority = p then (⟨b.priority, b.tasks ++ [t]⟩ : Bucket ν σ β ε δ ι α) :: rest
         else if p < b.priority then ⟨p, [t]⟩ :: b :: rest
         else b :: ((Effect4.Machine.Dispatcher.mk rest a).enqueue p t).buckets) := rfl
...
theorem bucketPairs_enqueue (a : Bool) (p : Nat) (t : Task ν σ β ε δ ι α) :
    ∀ bs : List (Bucket ν σ β ε δ ι α), strictAsc (bucketPairs bs) = true →
      bucketPairs ((Effect4.Machine.Dispatcher.mk bs a).enqueue p t).buckets
    ...
    · by_cases h2 : p < b.priority
      ...
      · rw [if_neg h1, if_neg h2, hhead]
        ...
        show (b.priority, (⟨b.tasks⟩ : Deque _))
            :: bucketPairs ((Effect4.Machine.Dispatcher.mk rest a).enqueue p t).buckets = _
...
theorem enqueue_wf (a : Bool) (p : Nat) (t : Task ν σ β ε δ ι α)
    ...
    strictAsc (bucketPairs ((Effect4.Machine.Dispatcher.mk bs a).enqueue p t).buckets) = true := by
  rw [bucketPairs_enqueue a p t bs hwf]
  exact strictAsc_insertIn _ _ hwf

/-- `Dispatcher.empty` satisfies it. -/
theorem empty_wf : Wf (Effect4.Machine.Dispatcher.empty (ν := ν) (σ := σ) (β := β)
    (ε := ε) (δ := δ) (ι := ι) (α := α)) := rfl

/-- The Deep dispatcher, read as a `Buckets`. -/
def toBuckets (d : Effect4.Machine.Dispatcher ν σ β ε δ ι α) (h : Wf d) :
    Buckets (Task ν σ β ε δ ι α) :=
  ⟨bucketPairs d.buckets, h⟩

/-- **`Effect4.Machine.Dispatcher.enqueue` is `Buckets.enqueue`, bundled.** -/
theorem toBuckets_enqueue (d : Effect4.Machine.Dispatcher ν σ β ε δ ι α) (h : Wf d)
...
/-- **`Effect4.Machine.Dispatcher.drain` is `Buckets.drain`.** -/
theorem drain_projection (d : Effect4.Machine.Dispatcher ν σ β ε δ ι α) (h : Wf d) :
    (Effect4.Machine.Dispatcher.drain d).1 = Buckets.drainTasks (toBuckets d h) := by
  rw [Buckets.drain_eq_flatten]
  show (d.buckets.map Effect4.Machine.Bucket.tasks).flatten = _
  simp only [toBuckets, bucketPairs, List.map_map]
  rfl

/-- And the Deep dispatcher is drained once, as `Buckets` is. -/
theorem drain_snd_empty (d : Effect4.Machine.Dispatcher ν σ β ε δ ι α) :
    (Effect4.Machine.Dispatcher.drain d).2 = Effect4.Machine.Dispatcher.empty := rfl
