...
theorem in `Test/Schema/RepresentationContract.lean` still passes
...
The fold of this family is generated, not written here: `src/Effect4/Schema/Fold.lean`
(the `SchemaFold` group of `tools/Effect4Gen/manifest.json`) emits `RepresentationAlgebra`,
`cata_representation`, `cata_check`, one `@[simp]` constructor equation per constructor
stated with the containers' own maps, `RepresentationHom` with its uniqueness theorem,
