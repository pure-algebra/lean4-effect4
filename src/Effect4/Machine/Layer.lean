...
namespace Effect4.Machine.Layers

open Effect4 Effect4.Machine Effect4.Machine.Env
...
abbrev LayerMachine := Effect4.Machine.RunMachine Name Thunk Val Err Defect FiberId Ann Ctx St
...
def runWitness (program : ProgName) : LayerMachine × ExitV :=
  Effect4.Machine.runSyncExit (interp witnessTable) witnessFuel
    (Effect4.Machine.RunMachine.empty St.empty) (progOf witnessTable program) Context.empty
...
def finProgCount (program : ProgName) : Nat :=
  ((runWitness program).1.trace.filter fun e =>
    match e with
    | Effect4.Machine.RunEvent.finalizerProgram _ _ _ => true
    | _ => false).length
