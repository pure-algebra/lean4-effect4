...
theorem denoteBWith_badShape (k : Nat) : ∀ (e : NativeEff) (env : List Val),
  ...
  | .succeed _, env | .fail _, env | .failCause _, env | .yieldError _, env | .sync _, env
  | .perform _ _, env | .gen _, env | .uninterruptible _, env | .interruptible _, env
  | .whileLoop _ _ _ _, env | .yieldNow _, env | .callback _ _, env | .awaitFiber _ _, env
...
theorem soundB (bad : ExitV) (k : Nat) : ∀ (e : NativeEff) (tys : TyEnv) (env : List Val)
  ...
  | .failCause v, tys, env, s, t, hl, hty, hat | .yieldError v, tys, env, s, t, hl, hty, hat
  ...
  | .interruptible _, _, _, _, _, hl, _, _ | .whileLoop _ _ _ _, _, _, _, _, hl, _, _
  | .yieldNow _, _, _, _, _, hl, _, _ | .callback _ _, _, _, _, _, hl, _, _
