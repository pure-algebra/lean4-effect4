...
* `scoped` — the `Scope` requirement the body
  carries is discharged using `bodyRequires`, matching the current checker.
...
inductive HasTy (sig : Signature Op) : TyEnv → Eff Op → EffTy → Prop
  ...
  /-- `Effect.scoped` (`:3960`): discharge the supplied Scope requirement (DI-63). -/
