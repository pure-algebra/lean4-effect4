...
theorem Looped.of_straight : ∀ (e : NativeEff), Straight e = true → Looped e = true
  ...
  | .succeed _, h | .fail _, h | .failCause _, h | .yieldError _, h | .sync _, h
  | .perform _ _, h | .gen _, h | .uninterruptible _, h | .interruptible _, h
  | .whileLoop _ _ _ _, h | .yieldNow _, h | .callback _ _, h | .awaitFiber _ _, h
...
theorem denoteB_mono :
  ...
  | .interruptible _, k, env, s, x, s', h | .whileLoop _ _ _ _, k, env, s, x, s', h
