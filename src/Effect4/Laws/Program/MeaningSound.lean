...
theorem denoteWith_badShape : ∀ (e : NativeEff) (env : List Val),
  ...
  | .succeed v, env | .fail v, env | .yieldError v, env => by
  ...
  | .gen _, _ | .uninterruptible _, _ | .interruptible _, _ | .whileLoop _ _ _ _, _
  | .iterate _ _ _ _ _ _, _ | .yieldNow _, _ | .callback _ _, _ | .awaitFiber _ _, _
...
theorem sound (bad : ExitV) : ∀ (e : NativeEff) (tys : TyEnv) (env : List Val) (s : Stores)
  ...
  | .yieldError v, tys, env, s, t, _, hty, hat => by
    obtain ⟨ty, hv, hadm, rfl⟩ := inv_yieldError nativeSignature tys v t hty
    obtain ⟨x, hx⟩ := Option.isSome_iff_exists.mp (evalTerm_isSome v env tys ty hat.fits hv)
    show SoundP _ _ s _ _
    rw [denoteWith, denote, hx]
    exact SoundP.pure hat.wf hat.heap _
  ...
  | .interruptible _, _, _, _, _, hs, _, _ | .whileLoop _ _ _ _, _, _, _, _, hs, _, _
  | .iterate _ _ _ _ _ _, _, _, _, _, hs, _, _ | .yieldNow _, _, _, _, _, hs, _, _
  | .callback _ _, _, _, _, _, hs, _, _ | .awaitFiber _ _, _, _, _, _, hs, _, _
