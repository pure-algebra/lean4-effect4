...
/-!
# Laws.Api.Player: refusal is a verdict, and journals act on players

* `step_refused`: a row whose phase is a refusal leaves the player exactly as it was. Each
  checked transition of `HostSession` answers its refusals with the session it was given, so a
  journal may hold refused rows and still replay to the same player.
* `replay_append`: playing `a ++ b` is playing `a`, then `b` from the player `a` reached, with
  the phases concatenated. Journals under concatenation act on players, which is what lets a
...
/-- A refused row leaves the player unchanged. -/
theorem step_refused (p : Player) (c : Command) (why : HostSession.Refusal)
  ...
  show ({ p with session := (result p c).session } : Player) = p
  rw [hs]

theorem replay_nil (p : Player) : replay p [] = (p, []) := rfl

theorem replay_cons (p : Player) (c : Command) (rest : List Command) :
    replay p (c :: rest) =
      ((replay (step p c).1 rest).1, (step p c).2 :: (replay (step p c).1 rest).2) := rfl

/-- Journals act on players: a concatenation plays as its parts, in order. -/
theorem replay_append (p : Player) (a b : List Command) :
    replay p (a ++ b) =
...
/-- The player a journal reaches is reached through any split of it. -/
theorem replay_append_player (p : Player) (a b : List Command) :
    (replay p (a ++ b)).1 = (replay (replay p a).1 b).1 := by
  rw [replay_append]

/-! ## The algebra

`step` is a Mealy machine on players: a coalgebra `Player → (Command → Player × Phase)`.
Three structures follow from it, and nothing else about a holder needs proving.

1. **Journals act.** A `Play` is a player transformer that also writes phases (a Kleisli arrow
...
3. **Behaviour.** `behaviour p` is what a player can be observed to do, the phases of every
   journal. It unfolds along `step` (`behaviour_cons`), which makes it the map into the final
   Mealy machine; two instances that agree on it are the same player to every holder.

One law joins the refusal to the algebra: a refused row is the unit. It can be dropped from a
journal without changing the player reached or any later phase (`replay_skip_refused`,
`behaviour_skip_refused`), so a journal has a normal form with no refused rows, which is what
a compaction may keep. -/

/-- A player transformer that writes the phases it passes through. -/
abbrev Play := Player → Player × List HostSession.Phase

/-- The play that does nothing. -/
def Play.id : Play := fun p => (p, [])

/-- One play, then another from the player the first reached; phases in order. -/
...
/-- What a player can be observed to do: the phases of every journal. -/
def behaviour (p : Player) (journal : List Command) : List HostSession.Phase :=
  (replay p journal).2

theorem behaviour_nil (p : Player) : behaviour p [] = [] := rfl

/-- Behaviour unfolds along `step`: the first phase, then the behaviour of the next player. -/
theorem behaviour_cons (p : Player) (c : Command) (rest : List Command) :
    behaviour p (c :: rest) = (step p c).2 :: behaviour (step p c).1 rest := rfl

/-- A refused row is the unit of the action: dropping it reaches the same player. -/
theorem replay_skip_refused (p : Player) (c : Command) (rest : List Command)
...
theorem behaviour_skip_refused (p : Player) (c : Command) (rest : List Command)
    (why : HostSession.Refusal) (h : (step p c).2 = .refused why) :
