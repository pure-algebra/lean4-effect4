...
Reachability fixes the program, table, compile budget, and initial
...
variable {p : Api.Program} {table : RowTable} {cf : Nat}
...
theorem guard_persists
    (reachable : Guard.Reachable p table cf answers m)
  ...
  Guard.requestOrInterrupted_steppedBy p table fuel m d key.fiber key.token r
    (Guard.guardState_reachable p table cf answers m reachable) request other
...
theorem guard_persists_single {f : Guard.NFiber}
  ...
  Guard.SingleGuard.requestOf_singleton_steppedBy p table fuel
    (Guard.guardState_reachable p table cf answers m reachable) single request d
...
theorem guard_persists_single_tape {f : Guard.NFiber}
  ...
  Guard.SingleGuard.requestOf_singleton_executePrefix p table
    (Guard.guardState_reachable p table cf answers m reachable) single request history
