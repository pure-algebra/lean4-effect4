...
/-!
# Api.Runner: a session as something that plays a journal

The session API of the runner plan (`docs/research/2026-09-17-runner-schema-codegen-plan.md`
...
* **One transition.** `step : Runner → Command → Runner × Phase` is total. A refusal is a
  verdict: the phase says why and the runner is unchanged (`step_refused`), so a journal may
  hold refused rows and replay still means the same thing.
* **No index.** `Runner` packs the program, the table and the session indexed by them, so a
...
* **Replay.** `replay` folds `step` over a journal and keeps every phase. It distributes over
  concatenation (`replay_append`): journals act on runners, and a runner reached by a prefix
...
/-- A runner from a program, its row table and an explicit header, or the refusal. -/
def load (program : Api.Program) (table : RowTable) (expectedProfile : String)
    (header : Header) (compileFuel stepFuel : Nat) : Except HostSession.Refusal Runner :=
...
def result (p : Runner) : Command → Result p.program p.table
...
/-- One row played: the next runner and the phase the row ended in. -/
def step (p : Runner) (c : Command) : Runner × HostSession.Phase :=
  let r := result p c
  ({ p with session := r.session }, r.phase)

/-- A journal played from a runner: the runner it reaches and the phase of every row. -/
def replay (p : Runner) : List Command → Runner × List HostSession.Phase
  | [] => (p, [])
  | c :: rest =>
...
def outstanding (p : Runner) : List Await := HostSession.outstanding p.session
