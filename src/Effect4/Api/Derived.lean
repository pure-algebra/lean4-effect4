...
--   lake env lean -M 4096 --run tools/Effect4Gen/Main.lean --group Api --imports Effect4.Api.Frontier,Effect4.Store.Domain.Canonical,Effect4.Store.Domain.Clock --out src/Effect4/Api/Derived.lean --append tools/Effect4Gen/guards/api.lean \
  ...
  ⟨.sum "FrontierReason"
     [("commandFuel", 0, []),
      ...
      ("awaitTimer", 3, [("fiber", (shape _root_.Effect4.FiberId).root),
        ("wakeAt", (shape _root_.Effect4.ClockMillis).root)]),
      ("awaitDecision", 4, [])],
...
def ofVal : Val → Option (_root_.Effect4.Api.FrontierReason)
  ...
  | .ctor 3 [v0, v1] =>
    match Canonical.ofVal (α := _root_.Effect4.FiberId) v0,
        Canonical.ofVal (α := _root_.Effect4.ClockMillis) v1 with
...
theorem fits (a : _root_.Effect4.Api.FrontierReason) : shapeDoc.accepts (toVal a) = true := by
  ...
  | «awaitTimer» a0 a1 =>
    exact accepts_sum _ _ _ 3 "awaitTimer" _ _ rfl
      (acceptsFields_cons _ _ _ _ _ _ (lift_FiberId a0)
        (acceptsFields_cons _ _ _ _ _ _ (lift_ClockMillis a1) (acceptsFields_nil _)))
  | «awaitDecision» =>
