...
A frame is `tag :: be64 length ++ payload` (`src/Effect4/Store/Domain/Canonical.lean:55-57`), and a
natural number is its digit string without a leading zero byte (`Canonical.lean:45-53`). Both
are read back by one fold, `natOfDigits` (`src/Effect4/Store/Domain/ProgramWire.lean:176-178`). The three
