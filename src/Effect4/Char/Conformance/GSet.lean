...
| | |
| --- | --- |
| Carrier | `GSet α`, a `List α` with `DecidableEq α`, `deriving DecidableEq, Repr`; `Canonical` and `Content` when `α` is canonical (`Char/Canonical.lean`) |
...
limit recorded in `docs/research/2026-09-05-workshop-char/10-conformance/08-open-questions.md`. Nothing
...
with the other generic ones in `src/Effect4/Char/Canonical.lean`, on the
`Prod`/`List` templates of `src/Effect4/Store/Canonical.lean:486-601`.
-/
