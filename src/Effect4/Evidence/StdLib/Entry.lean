...
`Tree` is the payload of a node at kind `tree`: name → reference pairs, the one way a name
reaches content now that the trie and its paths are retired (Q3, "names leave the store"). It
belongs beside `AnyRef` in `src/Effect4/Store/Node.lean` and is declared here, in the store's
namespace, only because this landing gives lane C no file under `src/Effect4/Store/`; moving the
declaration later changes no consumer.
-/
...
resolves at the kind it carries. -/
...
which is what a battery counts. Beside `Tree`, and for the same reason: the store's own modules
own no total combinator, and three families here build stores by folding one. -/
