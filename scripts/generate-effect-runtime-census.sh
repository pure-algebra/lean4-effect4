...
MutableRef.ts|0ededd9c6d3f865a9ff804aff3fe7ca7413c22ecd697ae0e7f87d80771ee7a1f
Deferred.ts|78b5d3cd2ad37f9e4f8ebaf465c9375bb982a00bd22a9f3d50ed02e0cb65f0e9
Layer.ts|55f20d4a18913efc16f8bd5732d477e9455fb2ea1476e47d0a4ed14b12caed58
internal/layer.ts|6ad3c8e779bae54dc0b3e57cd99fcd2087354df0c673d6335619d5cd95a74187
INPUTS
}
...
fork|fork.race-all|internal/effect.ts|export const raceAll = <Eff extends Effect.Effect<any, any, any>>(|-1|55|e1bfade75a179fe549bd6d2c2f8240e520afb9ec0cfaee109e3229314bc0627d|raceAll starts entrants in order as immediate daemons until observed success stops the start loop; the winning callback bypasses cleanup when its tracked set is empty, otherwise deferred uninterruptible interruption and waiting use that mutable set; all-failure reasons concatenate in completion order.
...
ref|ref.make|Ref.ts|export const makeUnsafe = <A>(value: A): Ref<A> => {|0|31|29cc798fadb78339ad4d745cecce74d1f8a3e43120341416f4a836f1bb2efd9e|A Ref is an object over RefProto whose single field is a fresh MutableRef cell, and make is Effect.sync over that constructor, so every evaluation of the same make effect allocates a distinct cell.
ref|ref.get|Ref.ts|export const get = <A>(self: Ref<A>) => Effect.sync(() => self.ref.current)|0|0|683d895e052c900a6a46c2396307ae4a7a20249841515993e347fbec85816312|get is a synchronous read of the cell's current field with no copy, so it observes every write already made through any holder of the same Ref.
ref|ref.set-void-returns-cell|Ref.ts|export const set = dual<|70|71|76183b70bc43231a87fd502c7aa56d5153e4bb35c05ef5ed652437ca581d99d2|set is declared Effect<void> but its sync thunk returns whatever MutableRef.set returns, so the runtime success value of Ref.set is the mutable cell rather than undefined.
ref|ref.cell-set-returns-self|MutableRef.ts|  <T>(self: MutableRef<T>, value: T): MutableRef<T>|0|7|5fb678545c31cb7212daad94ed4ebb121f376c971c77451787eb64977edaa0c8|MutableRef.set writes the current field and returns the ref itself, which is the value the void-typed Ref.set thunk produces.
ref|ref.get-and-set|Ref.ts|export const getAndSet = dual<|61|66|992fdabfde2339ae9c72e34031b33321074f9b007c1a6277f54927448efa62dc|getAndSet reads current, writes the new value, and returns the value read, all inside one sync thunk, so no other fiber can observe the state between the read and the write.
ref|ref.set-and-get-assignment|Ref.ts|export const setAndGet = dual<|69|69|1050895b7f665277dbe018851f5f18bb9497859c414335cdc052c942691d8caf|setAndGet succeeds with the value of the assignment expression itself rather than with a second read, so its result is the value written even if the cell is written again in the same tick.
ref|ref.update|Ref.ts|export const update = dual<|73|76|3bdef881a7d01f6af1131878e450914f2bfd111b1227175a7031fc6ba6d4da33|update applies the function to the current value and writes the result back in one sync thunk whose block returns nothing, so the effect succeeds with undefined and the function is applied exactly once per evaluation.
ref|ref.modify|Ref.ts|export const modify = dual<|99|104|f93ddd54b59f2dce84b2011a380e3abf85bcc80614b7021cb2e70bb50a857f0f|modify is the general read-modify-write shape: the function maps the current value to a pair, the second component is written back, and the first is the success value.
ref|ref.modify-some-no-reread|Ref.ts|export const modifySome: {|206|210|3ac78e8034b00e63abb6254d37250826d20f32a302be318ed814d0cf85cf7bb3|modifySome is defined as modify, and on a None second component it writes back the value modify already read rather than re-reading the cell.
ref|ref.update-some-and-get-reread|Ref.ts|export const updateSomeAndGet = dual<|87|94|a0708fc755fba13bbeec6a32e6a644199a24cc25f2d6c700fbc82401241c92aa|updateSomeAndGet writes only on Some and then succeeds with a fresh read of current, unlike getAndUpdateSome which succeeds with the value read before the write.
deferred|deferred.make|Deferred.ts|export const makeUnsafe = <A, E = never>(): Deferred<A, E> => {|0|5|0415dd162f2314578325cc4b79c9a93c474e3804c38dcc286fc9a53a7200984f|A fresh Deferred is an object over DeferredProto with both its waiter array and its completion effect undefined; there is no separate pending tag.
deferred|deferred.is-done|Deferred.ts|export const isDoneUnsafe = <A, E>(self: Deferred<A, E>): boolean => self.effect !== undefined|0|0|06d9574d884ee5d1750e2ccd84cb1a5cc58ed8e1e822e15b9b29308ccf34be55|Done-ness is exactly the presence of a stored completion effect, so the state space is undefined or one effect and nothing else.
deferred|deferred.await|Deferred.ts|const _await = <A, E>(self: Deferred<A, E>): Effect<A, E> =>|0|13|fca569e241a8c8c69abc6fdfee1544835081bfe27102aee07bb17df9dd6373e9|await is a callback effect that resumes at once with the stored effect when the Deferred is already done, and otherwise lazily creates the waiter array, appends its own resume, and returns a cleanup that splices that resume out and does nothing once completion has cleared the array.
deferred|deferred.single-completion|Deferred.ts|export const doneUnsafe = <A, E>(self: Deferred<A, E>, effect: Effect<A, E>): boolean => {|0|2|9a01f00c544fa1a722c2e762a00e147bbdda33e6de10409e6a3c20388b3f2696|A completion attempt answers false and changes nothing when an effect is already stored, so the first completion wins and every later one is a no-op.
deferred|deferred.completion-order|Deferred.ts|    // Clear `resumes` before resuming: a waiter resumed with an interrupt|-1|10|123ff15571cf7f2f9dc1fe2d17e93fdae1900b2684cb2e2ae3e67db9862785ac|Completion takes the waiter array and clears the field before resuming, then resumes every waiter in registration order with the stored effect and answers true.
deferred|deferred.complete-with-stores-effect|Deferred.ts|    internalEffect.sync(() => doneUnsafe(self, effect))|-4|1|0af894d0be2d85ff4aa9573d3116fba0984bd54cb7d2d948f330b0689a1481ab|completeWith stores the given effect as the completion without running it, so awaiters each run that effect themselves rather than sharing one result.
deferred|deferred.done-is-complete-with|Deferred.ts|} = completeWith as any|-1|0|4b35980b01397033eae6374e31448ff2983cf714237f5fb041af56d2d663bc60|done is completeWith itself: completing from an Exit stores that Exit as the completion effect, which is why an Exit completion is shared while an arbitrary effect completion is not.
deferred|deferred.complete-runs-once|Deferred.ts|    internalEffect.suspend(() => self.effect ? internalEffect.succeed(false) : into(effect, self))|-4|1|0ce966f621a4742a75f1208c8b354ad0676281f8aa613e958788a12c112088b9|complete suspends so the done check happens at run time, answers false without running the effect when already done, and otherwise runs it through into so its result is memoized.
deferred|deferred.into-uninterruptible|Deferred.ts|    internalEffect.uninterruptibleMask((restore) =>|-4|6|e8693a32653f082c61fbf0acd551dc748a456998767228932365e24436471703|into runs the body under an uninterruptible mask with interruptibility restored only inside, takes the body's Exit, and completes the Deferred with that Exit, so an interrupted body still completes the Deferred.
deferred|deferred.interrupt|Deferred.ts|export const interrupt = <A, E>(self: Deferred<A, E>): Effect<boolean> =>|0|1|baca35a99204de94fa1d626557a0b5930ad399297d6ef6b4b22841ebdc0ad72f|interrupt reads the running fiber's id through withFiber and delegates to interruptWith, so the recorded interruptor is the completing fiber, not the awaiting one.
deferred|deferred.interrupt-with|Deferred.ts|    failCause(self, internalEffect.causeInterrupt(fiberId))|-4|1|ae587496d012d68bcb3c4b4ea2bd4204a692fed32232e713382f3c2e72b6dc89|interruptWith completes by failCause of an interrupt cause carrying the given fiber id, so an interrupted Deferred is an ordinary stored failure completion and not a distinguished state.
deferred|deferred.poll|Deferred.ts|export function poll<A, E>(self: Deferred<A, E>): Effect<Option.Option<Effect<A, E>>> {|0|2|d46fc514aa4b7d86745e51692ac8a0fdea78ad8de1e515bb95f2878f51a89789|poll is a non-blocking sync read that maps the undefined completion slot to None and a stored completion to Some of that effect.
layer|layer.from-build-unsafe|Layer.ts|const fromBuildUnsafe = <ROut, E, RIn>(|0|9|0ee0ef030418cbf4d5c6d02ff4d3685f92ed5fd3c5e4e01f413529d533439e2f|A Layer is an object over LayerProto whose only field is a build function of a memo map and a scope; fromBuildUnsafe installs that function with no scope handling of its own.
layer|layer.from-build-child-scope|Layer.ts|export const fromBuild = <ROut, E, RIn>(|0|12|59825bfe960cbbdf35bfdeaf94b4ca168e876eead5a6cbd909cd36db0dfcbc4e|fromBuild forks a child layer scope from the caller scope for the build and closes that child scope only when the build exits in Failure, so a successful build leaves its finalizers attached to the caller scope.
layer|layer.build-with-memo-map-service|Layer.ts|} = dual(3, <RIn, E, ROut>(|0|9|df77c8fd319fc35954dac51c5855091dcda674f6b29214200e633943c4a188ba|buildWithMemoMap runs the layer's build with the memo map installed as the CurrentMemoMap service and adds that same memo map to the produced context, so nested builds inherit it.
layer|layer.memo-build-once|Layer.ts|const memoMapBuild = <RIn, E, ROut>(|0|29|85ce4b7ec4a4a03f19c252470c576e350b0b13f6a528c2d8fd4e45b8c50299da|The first build of a layer in a memo map allocates a root layer scope and a Deferred, stores an entry with one observer whose effect is that Deferred's await, registers the entry finalizer on the caller scope, builds into the layer scope, and on exit replaces the entry effect with the exit and completes the Deferred, so every other observer shares one build.
layer|layer.memo-finalizer-last-observer|Layer.ts|        entry.observers--|-2|7|a8b26eb3d7733f6b3707df36fb1ac4e0f23f2bb3b937baa7b6bf23adb8ce01f1|The memo entry finalizer decrements the observer count on every close, and only the close that brings it to zero deletes the entry from the memo map and closes the layer scope with the closing exit; every other close is void.
layer|layer.memo-reuse-observer-count|Layer.ts|const memoMapReuse = <RIn, E, ROut>(|0|9|7cd74bcb322b5958f09fbceb66cb32287e1a414ea83e335972479503f3f8ff42|A memo hit increments the entry observer count and registers the same entry finalizer on the new caller scope before yielding the memoized effect, so reuse rebuilds nothing and adds one more close that must happen before the layer scope closes.
layer|layer.memo-map-parent-lookup|Layer.ts|  get<RIn, E, ROut>(|0|9|978662851872ea543b964f2f470265e3c35a75269a56c992d3c0233e1d19ade0|A memo map answers from its own map first, reusing that entry, and otherwise delegates to its parent memo map, so a forked memo map sees layers already built by its parent.
layer|layer.memo-get-or-else|Layer.ts|  getOrElseMemoize<RIn, E, ROut>(|0|12|9dcb871ddb5de3167b0f05337f663e70d574781d85a7bd32ca6578c3b00c03e8|getOrElseMemoize suspends so the lookup happens at run time, returns the memoized effect on a hit, and otherwise starts exactly one memoMapBuild for that layer.
layer|layer.current-memo-map-fork-or-create|Layer.ts|  static forkOrCreate<Services>(self: Context.Context<Services>): MemoMap {|0|3|877ec175e00a49a233fb96becc08f472b23ad243e05dbcba1ec04cbe8760a675|The memo map for a build is forked from the fiber's CurrentMemoMap when one is present and created fresh otherwise, so an outer build is shared only through the parent chain and never written to.
layer|layer.build-uses-ambient-scope|Layer.ts|export const build = <RIn, E, ROut>(|0|9|269fa81994ef41ef66c50ba0181589657ea10c66a07311ece95b5b4dc07a9e69|Layer.build reads both inputs from the running fiber's context: the memo map by forkOrCreate and the scope by an unchecked Scope service lookup, so building without an ambient Scope is a host defect rather than a typed error.
layer|layer.build-with-scope-still-forks-memo|Layer.ts|} = dual(2, <RIn, E, ROut>(|0|10|d567ddf02039015ac75d0d9d32077044109293dc6fdb0131656ada5a4a529878|buildWithScope replaces only the scope; the memo map is still forked or created from the fiber context, so an explicit scope does not imply an explicit memo map.
layer|layer.merge-parallel-scopes|Layer.ts|const mergeAllEffect = <Layers extends [Layer<never, any, any>, ...Array<Layer<never, any, any>>]>(|0|15|1d0b3d3c349f959454031d1f20381526412aa4e24b1c37275588e3e089be1e8c|Merging forks one parallel-strategy scope from the caller scope, gives every merged layer its own sequential child of it, builds them all with concurrency equal to the layer count over one shared memo map, and merges the resulting contexts.
layer|layer.provide-dependency-first|Layer.ts|const provideWith = (|0|19|1ae74e2679e2caeca59668d10f285320fae7b25383582d94bbb4d7ffde575756|provide builds the dependency first on the same memo map and the same scope, provides that context to the dependent layer's build, and combines the two contexts with a function that is identity for provide, so the dependency services do not reach the caller.
layer|layer.fresh-drops-memoization|Layer.ts|export const fresh = <A, E, R>(self: Layer<A, E, R>): Layer<A, E, R> =>|0|1|ed22d78ebc3b4595dde975b9795be8c5cbc535e9591e1c78bc29f0c4dcec84ba|fresh calls the layer's build directly with a brand new memo map, so the layer is rebuilt even where the ambient memo map already holds it and it gets no fromBuild child scope of its own.
layer|layer.launch-holds-scope|Layer.ts|export const launch = <RIn, E, ROut>(self: Layer<ROut, E, RIn>): Effect<never, E, RIn> =>|0|1|1389c935e46ef059f3b6c34ba0451f7d6543a1fea5c9519108cff34cd7b53988|launch builds the layer inside a fresh scope and then runs never, so the built layer stays alive until that effect is interrupted and the scope close runs the acquisition finalizers.
layer|layer.provide-effect-scope|internal/layer.ts|const provideLayer = <A, E, R, ROut, E2, RIn>(|0|14|4f1f46040fd784e2c2d704b1b9b69bd97d14b37edb5a52559a1d04483d78044a|Providing a layer to an effect opens a fresh scope around that effect, builds the layer into it, and provides the resulting context, so the layer's lifetime is exactly the provided program's scope; the local option builds through a private memo map instead of the ambient one.
ROWS
}
...
unknown_kind="$(awk -F '\t' '
  BEGIN {
    split("op frame-arm checkpoint interrupt fork scope scheduler exit cause entry rule ref deferred layer", ks, " ")
