...
ref|ref.get-and-set|Ref.ts|export const getAndSet = dual<|61|66|992fdabfde2339ae9c72e34031b33321074f9b007c1a6277f54927448efa62dc|getAndSet reads current, writes the new value, and returns the value read, all inside one sync thunk, so the read and the write happen with no intervening runtime step.
ref|ref.set-and-get-assignment|Ref.ts|export const setAndGet = dual<|69|69|1050895b7f665277dbe018851f5f18bb9497859c414335cdc052c942691d8caf|setAndGet succeeds with the value of the assignment expression itself rather than with a second read of the cell.
...
layer|layer.build-uses-ambient-scope|Layer.ts|export const build = <RIn, E, ROut>(|0|9|269fa81994ef41ef66c50ba0181589657ea10c66a07311ece95b5b4dc07a9e69|Layer.build reads both inputs from the running fiber's context: the memo map by forkOrCreate and the scope by an unchecked Scope service lookup, so a build with no ambient Scope fails in that host lookup rather than as a typed error.
