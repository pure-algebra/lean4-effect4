(* cut-from: rev=ac07384-dirty toolchain=4.33.1 inputs=1a79bb92fe1dfeed72771bd4987cc2967647b0b93810189ac1761304cea9ea3d
...
let rec program_ty_normalize (x_1 : ty) : ty =
  match (x_1 : ty) with
    | Ty_option inner_2 -> (let _x_3 = program_ty_normalize inner_2 in
      let _x_4 = Ty_option _x_3 in
      _x_4)
    | Ty_list inner_5 -> (let _x_6 = program_ty_normalize inner_5 in
      let _x_7 = Ty_list _x_6 in
      _x_7)
    | Ty_prod (left_8, right_9) -> (let _x_10 = program_ty_normalize left_8 in
      let _x_11 = program_ty_normalize right_9 in
      let _x_12 = Ty_prod (_x_10, _x_11) in
      _x_12)
    | Ty_except (error_13, value_14) -> (let _x_15 = program_ty_normalize error_13 in
      let _x_16 = program_ty_normalize value_14 in
      let _x_17 = Ty_except (_x_15, _x_16) in
      _x_17)
    | Ty_exitOf (value_18, error_19) -> (let _x_20 = program_ty_normalize value_18 in
      let _x_21 = program_ty_normalize error_19 in
      let _x_22 = Ty_exitOf (_x_20, _x_21) in
      _x_22)
    | Ty_causeOf error_23 -> (let _x_24 = program_ty_normalize error_23 in
      let _x_25 = Ty_causeOf _x_24 in
      _x_25)
    | Ty_fiberOf (value_26, error_27) -> (let _x_28 = program_ty_normalize value_26 in
      let _x_29 = program_ty_normalize error_27 in
      let _x_30 = Ty_fiberOf (_x_28, _x_29) in
      _x_30)
    | Ty_union (left_31, right_32) -> (let _x_33 = program_ty_normalize left_31 in
      let _x_34 = program_ty_members _x_33 in
      let _x_35 = program_ty_normalize right_32 in
      let _x_36 = program_ty_members _x_35 in
      let _x_37 = _x_34 @ _x_36 in
      let _x_38 = row_normalize_at_program_ty_normalize_spec_0 _x_37 in
      let _x_39 = program_ty_of_members _x_38 in
      _x_39)
    | _ -> x_1
...
let rec program_val__has_ty (v : val_) (ty : ty) (allocated : string list) : bool =
  match (ty : ty) with
    ...
    | Ty_bool -> (match (v : val_) with
        | Val_bool _ -> (let _x_7 = true in
          _x_7)
        | _ -> (let _x_8 = false in
          _x_8))
    | Ty_string -> (match (v : val_) with
        | Val_str _ -> (let _x_10 = true in
          _x_10)
        | _ -> (let _x_11 = false in
          _x_11))
    | Ty_option inner_12 -> (match (v : val_) with
        | Val_none -> (let _x_13 = true in
          _x_13)
        | Val_some a_14 -> (let _x_15 = program_val__has_ty a_14 inner_12 allocated in
          _x_15)
        | _ -> (let _x_16 = false in
          _x_16))
    | Ty_handle target_17 -> (match (v : val_) with
        | Val_handle (kind_18, key_19) -> (let _x_20 = handle_kind_of_byte_opt kind_18 in
          match _x_20 with
            | Some val__21 -> (match (val__21 : handle_kind) with
                | HandleKind_cell -> (let _x_22 = "Ref.Ref<number>" in
                  let _x_23 = target_17 = _x_22 in
                  _x_23)
                | HandleKind_promise -> (let _x_24 = "Deferred.Deferred<number, number>" in
                  let _x_25 = target_17 = _x_24 in
                  _x_25)
                | HandleKind_scope -> (let _x_26 = "Scope.Scope" in
                  let _x_27 = target_17 = _x_26 in
                  _x_27)
                | HandleKind_external -> (let _x_28 = program_external_handle_target target_17 in
                  if _x_28 then (let _x_29 = list_get_opt_internal allocated key_19 in
                    let _x_30 = Some target_17 in
                    let _x_31 = option_inst_beq_beq_at_std_http_header_transfer_encoding_validate_spec_0 _x_29 _x_30 in
                    _x_31) else _x_28)
                | _ -> (let _x_32 = false in
                  _x_32))
            | _ -> (let _x_33 = false in
              _x_33))
        | _ -> (let _x_34 = "Context.Context<unknown>" in
          let _x_35 = target_17 = _x_34 in
          if _x_35 then (let _x_36 = val__context_opt v in
            match _x_36 with
              | None -> (let _x_37 = false in
                _x_37)
              | Some _ -> _x_35) else _x_35))
    | Ty_fiberOf (_, _) -> (match (v : val_) with
        | Val_handle (kind_41, _) -> (let _x_43 = 1 in
          let _x_44 = kind_41 = _x_43 in
          _x_44)
        | _ -> (let _x_45 = false in
          _x_45))
    | Ty_exitOf (value_46, error_47) -> (match (v : val_) with
        | Val_ctor (index_48, args_49) -> (let _x_50 = 0 in
          let _x_51 = index_48 = _x_50 in
          if _x_51 then (match args_49 with
              | head_61 :: tail_62 -> (match tail_62 with
                  | [] -> (let _x_63 = program_val__has_ty head_61 value_46 allocated in
                    _x_63)
                  | _ -> (let _x_64 = false in
                    _x_64))
              | _ -> (let _x_65 = false in
                _x_65)) else (let _x_52 = 1 in
            let _x_53 = index_48 = _x_52 in
            if _x_53 then (match args_49 with
                | head_54 :: tail_55 -> (match tail_55 with
                    | [] -> (let _x_56 = cause_image in
                      match (_x_56 : _ image) with
                        | { of_val = of_val; _ } -> (let _x_57 = of_val head_54 in
                          match _x_57 with
                            | None -> _x_51
                            | Some val__58 -> (let _f_59 = program_val__has_ty__lam_0 error_47 allocated in
                              let _x_60 = list_all_at_program_cause_admits_spec_0 _f_59 error_47 val__58 in
                              _x_60)))
                    | _ -> _x_51)
                | _ -> _x_51) else _x_53))
        | _ -> (let _x_66 = false in
          _x_66))
    | Ty_causeOf error_67 -> (let _x_68 = val__cause_opt v in
      match _x_68 with
        | None -> (let _x_69 = false in
          _x_69)
        | Some val__70 -> (let _f_71 = program_val__has_ty__lam_0 error_67 allocated in
          let _x_72 = list_all_at_program_cause_admits_spec_0 _f_71 error_67 val__70 in
          _x_72))
    | Ty_prod (left_73, right_74) -> (match (v : val_) with
        | Val_list xs_75 -> (match xs_75 with
            | head_76 :: tail_77 -> (match tail_77 with
                | head_78 :: tail_79 -> (match tail_79 with
                    | [] -> (let _x_80 = program_val__has_ty head_76 left_73 allocated in
                      if _x_80 then (let _x_81 = program_val__has_ty head_78 right_74 allocated in
                        _x_81) else _x_80)
        ...
        | _ -> (let _x_85 = false in
          _x_85))
    | Ty_list inner_86 -> (match (v : val_) with
        | Val_ctor (index_87, args_88) -> (let _x_89 = 3 in
          let _x_90 = index_87 = _x_89 in
          if _x_90 then (match args_88 with
              | _ :: tail_92 -> (match tail_92 with
                  | [] -> (let _x_93 = val__snapshot_opt v in
                    match _x_93 with
                      | None -> (let _x_94 = false in
                        _x_94)
                      | Some val__95 -> (let _x_96 = list_all_at_program_val__has_ty_spec_0 inner_86 allocated val__95 in
                        _x_96))
                  | _ -> (let _x_97 = false in
                    _x_97))
              | _ -> (let _x_98 = false in
                _x_98)) else _x_90)
        | Val_list xs_99 -> (let _x_100 = list_all_at_program_val__has_ty_spec_1 inner_86 allocated xs_99 in
          _x_100)
        | _ -> (let _x_101 = false in
          _x_101))
    | Ty_union (left_102, right_103) -> (let _x_104 = program_val__has_ty v left_102 allocated in
      if _x_104 then _x_104 else (let _x_105 = program_val__has_ty v right_103 allocated in
        _x_105))
...
let program_exit_scoped (root : native_op eff) (m : (eff_name, eff_thunk, val_, err, defect, int, unit, ctx, stores, (eff_name, eff_thunk, val_, err, defect, int, unit) prim, (eff_name, eff_thunk, val_, err, defect, int, unit) frame_fiber, (eff_name, eff_thunk, val_, err, defect, int, unit) frame_event) run_machine) (f : (eff_name, eff_thunk, val_, err, defect, int, unit, ctx, (eff_name, eff_thunk, val_, err, defect, int, unit) prim, (eff_name, eff_thunk, val_, err, defect, int, unit) frame_fiber) run_fiber) (yielding : bool) (exit_ : (val_, err, defect, int, unit) exit_) : (eff_name, eff_thunk, val_, err, defect, int, unit, ctx, stores, (eff_name, eff_thunk, val_, err, defect, int, unit) prim, (eff_name, eff_thunk, val_, err, defect, int, unit) frame_fiber, (eff_name, eff_thunk, val_, err, defect, int, unit) frame_event) iter =
  match (f : (_, _, _, _, _, _, _, _, _, _) run_fiber) with
    | { id = id; frame = frame; running = running; parked = parked; pending = pending; finalizing = finalizing; exit_ = exit__1; current_op_count = current_op_count; yield_override = yield_override; observers = observers; children = children; dispatcher = dispatcher; _ } -> (let _jp_1 = fun _y_2 _y_3 _y_4 _y_5 _y_6 _y_7 _y_8 _y_9 _y_10 -> let _x_11 = ({ current = _y_10; stack = _y_7; interruptible = _y_4; interrupted_cause = _y_5; deferred_interrupt = _y_2 } : (_, _, _, _, _, _, _) frame_fiber) in
      let _x_12 = ({ id = id; frame = _x_11; running = running; parked = parked; pending = pending; finalizing = finalizing; exit_ = exit__1; current_op_count = current_op_count; max_ops_before_yield = _y_9; prevent_yield = _y_6; yield_override = yield_override; observers = observers; children = children; dispatcher = dispatcher; context = _y_8 } : (_, _, _, _, _, _, _, _, _, _) run_fiber) in
                                        ...
                                        | fst_33, snd_34 -> (let m_2 = ({ fibers = fibers; races = races; next_id = next_id; next_token = next_token; next_race = next_race; middleware_installed = middleware_installed; armed = armed; state = fst_33; trace = trace; stuck = stuck } : (_, _, _, _, _, _, _, _, _, _, _, _) run_machine) in
                                          match snd_34 with
                                            | None -> (let _x_35 = prim_of_exit exit_ in
                                              _jp_1 deferred_interrupt m_2 interruptible interrupted_cause prevent_yield stack previous_25 max_ops_before_yield _x_35)
                                            | Some val__36 -> (let _x_37 = RunEvent_finalizerProgram (id, finalizer_23, exit_) in
                                              ...
                                              _jp_1 deferred_interrupt _x_39 interruptible interrupted_cause prevent_yield stack previous_25 max_ops_before_yield _x_41)))))))
