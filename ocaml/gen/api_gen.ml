(* cut-from: rev=5948229-dirty toolchain=4.33.1 inputs=7637411ad25578a423202fa168b5cbbe0d4336c7cec08972d188ab47bba3051a
  ...
  match (ty : ty) with
    ...
    | Ty_prod (left_55, right_56) -> (match (v : val_) with
        ...
        | _ -> (let _x_67 = false in
          _x_67))
    | Ty_except (_, _) -> (let _x_70 = false in
      _x_70)
    | Ty_exitOf (value_71, error_72) -> (match (v : val_) with
        | Val_ctor (index_73, args_74) -> (let _x_75 = 0 in
          let _x_76 = index_73 = _x_75 in
          if _x_76 then (match args_74 with
              | head_86 :: tail_87 -> (match tail_87 with
                  | [] -> (let _x_88 = program_val__has_ty head_86 value_71 allocated in
                    _x_88)
                  | _ -> (let _x_89 = false in
                    _x_89))
              | _ -> (let _x_90 = false in
                _x_90)) else (let _x_77 = 1 in
            let _x_78 = index_73 = _x_77 in
            if _x_78 then (match args_74 with
                | head_79 :: tail_80 -> (match tail_80 with
                    | [] -> (let _x_81 = cause_image in
                      match (_x_81 : _ image) with
                        | { of_val = of_val; _ } -> (let _x_82 = of_val head_79 in
                          match _x_82 with
                            | None -> _x_76
                            | Some val__83 -> (let _f_84 = program_val__has_ty__lam_0 error_72 allocated in
                              let _x_85 = list_all_at_program_cause_admits_spec_0 _f_84 error_72 val__83 in
                              _x_85)))
                    | _ -> _x_76)
                | _ -> _x_76) else _x_78))
        | _ -> (let _x_91 = false in
          _x_91))
    | Ty_causeOf error_92 -> (let _x_93 = val__cause_opt v in
      match _x_93 with
        | None -> (let _x_94 = false in
          _x_94)
        | Some val__95 -> (let _f_96 = program_val__has_ty__lam_0 error_92 allocated in
          let _x_97 = list_all_at_program_cause_admits_spec_0 _f_96 error_92 val__95 in
          _x_97))
    | Ty_fiberOf (_, _) -> (match (v : val_) with
        | Val_handle (kind_100, _) -> (let _x_102 = 1 in
          let _x_103 = kind_100 = _x_102 in
          _x_103)
        | _ -> (let _x_104 = false in
          _x_104))
    | Ty_union (left_105, right_106) -> (let _x_107 = program_val__has_ty v left_105 allocated in
      if _x_107 then _x_107 else (let _x_108 = program_val__has_ty v right_106 allocated in
        _x_108))
    | Ty_lit value_109 -> (match (v : val_) with
        | Val_str s_110 -> (let _x_111 = s_110 = value_109 in
          _x_111)
        | _ -> (let _x_112 = false in
          _x_112))
    | _ -> (let _x_113 = false in
      _x_113)
