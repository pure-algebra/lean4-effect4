...
#
# SUPERSEDED by dune since 2026-09-04 (`avatar/dune`, `ocaml/server/BUILD-DUNE.md`): build
# with `cd ocaml && dune build avatar`. Kept as the reference for what the shell build did
# and for the golden comparison below, which the dune build has no equivalent of. The
# toolchain now comes from the `effect4` opam switch on the PATH rather than a Mac prefix.
set -euo pipefail
