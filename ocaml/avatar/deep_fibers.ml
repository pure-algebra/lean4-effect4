(* The avatar: `src/Effect4/Machine/Fibers.lean` transcribed into OCaml 5 effect handlers.
   ...
   DIVERGENCE 2 (`Cmd.loop`, `Cmd.deliver`, `Cmd.finish`). Lean's `Cmd` (`Fibers.lean:511-537`)
   has eight arms and the avatar's `cmd` has five. Arm by arm, against the Lean model of
   2026-09-04 (the projection guard's `Fibers.cmd` row, `DerivedCheck.lean`):
     * `evaluate fiber`, `resume fiber token answer`, `launch race`, `link mode scope key
       target interruptor extra` and `drainDue` are `Cevaluate`, `Cresume`, `Claunch`,
       `Clink` and `CdrainDue`, arity for arity. `launch` lost its `entrant` argument in
       `57924eb` (R2-11: an entrant has no id before its launch); `Claunch` never carried
       one, so nothing moved here.
     * `loop fiber yielding` has no OCaml existence. Lean's `drive` runs one `iteration` per
       `Cmd.loop`; here `continue k` runs the fiber to its next `perform`. The `yielding`
       latch lives on the fiber (`run_fiber.yielding`), and `iteration`'s prelude (the
       deferred-interrupt check, the op counter, the yield injection) is `guard` /
       `resume_iteration` below, run at the head of every handler arm and of every resume --
       so the op counter counts performs, not `Prim` nodes.
     * `deliver fiber yielding` (`:933-934`, R2-1) is the function `deliver` below, not a
       command. The second half of `Sync[evaluate]` pops the continuation, and the avatar's
       continuation is the OCaml `k` the store arm holds (DIVERGENCE 1) -- a value no `cmd`
       of Lean's shape could carry. The arm runs what the thunk owed (`drive [CdrainDue]`)
       and then `deliver`: the same point in the order as Lean's `nested ++ [Cmd.deliver
       …]` (DIVERGENCE 4), with the same `getCont`-time deferred-interrupt check.
     * `finish fiber exit` (`:611-628`, M1) is the function `finish` below, not a command.
       It runs from `retc`/`exnc` once the fiber's own stack has returned, which is after
       the nested commands of its last primitive ran inline (DIVERGENCE 4), so the order
       Lean states -- what the last primitive owed, then the exit path -- holds; and it
       reads the live fiber record (DIVERGENCE 3), so an interrupt a nested command recorded
       is seen, as Lean's re-read of the fiber sees it. A `Cfinish` value would have no
       construction site: nothing in the avatar enqueues an exit path.
   Decision (2026-09-04, the drift re-diff): no `Cloop`, `Cdeliver` or `Cfinish` constructor
   is added; each is covered by the control flow above. The hand description `Avatar.cmd`
   (`Render.lean`) lists all eight Lean arms, the three absent ones carrying a comment that
   says so, so the projection guard is green and the divergence is stated, not absorbed.
...
(* `RunEvent` (`Fibers.lean:268-290`), one constructor per Lean constructor: 21 since
   `57924eb` (R2-11), `raceStarted` carrying the entrant *count* and `raceSkipped` retired --
   no entrant exists before its launch, so none is ever skipped. *)
type run_event =
...
(* `Cmd` (`Fibers.lean:511-537`), five of the eight Lean arms: `Cmd.loop`/`deliver`/`finish`
   are absent by DIVERGENCE 2, arm by arm at the top of this file (`deliver` and `finish` are
   the functions of those names). `Claunch race` is one iteration of `raceAll`'s register
   loop (`57924eb`, R2-11; the race alone, as Lean's `launch (race : Nat)`); `Clink (mode,
   scope, key, target, interruptor, extra)` is `forkIn`'s link after the child's start
   (R2-8). *)
type cmd =
  | Cevaluate of int
...
type _ Effect.t +=
  ...
  (* `WithFiberAction.awaitAllFailFast targets` (`Fibers.lean:232-234`, `:948-950`;
     `Effect.all`/`forEach` with concurrency, `Layer.ts:1597-1598`): `awaitAll` whose first
     failing exit interrupts the outstanding targets (the drift re-diff of 2026-09-04). *)
  | Op_await_fibers_fail_fast : int list -> exitv list Effect.t            (* .awaitAllFailFast *)
...
(* P5's generated corpus (`ocaml/probes/fuzz/corpus/corpus_fixture.ml`) registers itself
  ...
  match_with body ()
    {
      ...
      effc =
        (fun (type a) (eff : a Effect.t) : ((a, unit) continuation -> unit) option ->
          ...
          | Op_await_fibers_fail_fast targets ->
            Some (fun k -> dispatch_exits k (fun ko -> arm_await_fibers_fail_fast m f targets ko))
...
(* `WithFiberAction.awaitAllFailFast` (`Fibers.lean:948-950`; `Effect.all`/`forEach` with
   concurrency, `Layer.ts:1597-1598`): `awaitAll` on a countdown whose `failFast` flag is set
   (`Pending.failFast`, `Fibers.lean:83-85`). The walk and the park are `awaitAll`'s; the
   difference is in `fire_observer`'s `Countdown` arm (`Fibers.lean:1064-1071`): the first
   failing exit the countdown *observes* -- a failure among the exits the walk collected at
   park time does not count, and neither does a second one -- interrupts every target not
   yet visited, with the awaiter's id and stack annotations, in list order; the countdown
   still waits for each of them and answers the exits in input order (M6). Lean reaches it
   through `Layer.lean:348`'s `ActionName.awaitAllFailFast` (`mergeForkAll`, `:839`);
   `Stores.lean`'s `ActionName` has no such arm, so no `prog_name` of the store alphabet
   names it, and the avatar's Layer `action_of` refuses wholesale (`χ` is `unit`). It is
   reached as this effect. Added by the drift re-diff of 2026-09-04. *)
and arm_await_fibers_fail_fast m f (targets : int list) (ko : exitv list kops) : unit =
  begin
    let resume_answer (a : answer) =
      match a with
      | Aexits exits -> ko.ret exits
      ...
      | Acause c -> ko.thr (Einterrupt_exn c)
    in
    match countdown_park ~fail_fast:true m f targets RexitsValue ~on_answer:resume_answer with
    | Cparked _ -> ()
    | Cdone exits -> resume_answer (resume_prim RexitsValue exits)
  end
