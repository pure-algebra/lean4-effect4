...
#
# SUPERSEDED for the OCaml hosts by `tools/run-corpus-wsl.sh` (native + bytecode over dune's
# binaries) and `tools/run-corpus-jsoo.ps1` (the js host under the Windows node), which is
# what runs on the PC. This script still expects `build-avatar.sh`'s shell build in
# `$out/build` (`avatar.byte`, `avatar.native`, `avatar.js`) and the rc.112 package; its
# rc.112 half is re-spelled for the PC in `tools/run-corpus-rc112.ps1`.
set -uo pipefail
here=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
# <repo>/ocaml/avatar -> <repo>. The estate lived under `workshop/OCaml5/` until 2026-09-04.
repo=$(CDPATH= cd -- "$here/../.." && pwd)
out=${A0_BUILD:-${TMPDIR:-/tmp}/effect4-avatar-shell-build}
