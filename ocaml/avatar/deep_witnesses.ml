...
type snap = {
  ...
  refs : value list;
  cells : (program option * waiter_pair list) list;  (* the slot is Lean's `Option Program` *)
...
let completion_of (s : snap) (cell : int) : program option option =
  Option.map fst (List.nth_opt s.cells cell)

(* The stored program's exit: a closure cannot be compared, its exit can (`exit_of_run` is
   `Prim.exitFrame` on this stack; exact for the `Prim.ofExit` programs the store holds). *)
let completion_exit_of (s : snap) (cell : int) : exitv option option =
  Option.map (Option.map exit_of_run) (completion_of s cell)
...
let witnesses : witness list =
  [
    ...
    theorem "w10_into_completes_on_failure" "Witnesses.lean:818" [ ("w10Into", w10_into) ]
      (fun d ->
        let s = d "w10Into" in
        (* The slot holds the stored program, `Prim.ofExit (Exit.failure (Cause.fail Err.boom))`
           as Lean holds it, read back through its exit. *)
        expect (exit_of s 0 = Some (Esuccess (Vbool true)) && completion_exit_of s 0 = Some (Some (Efailure (cause_fail boom)))) "`into` did not answer true and store the failed exit");
