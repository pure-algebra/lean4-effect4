...
(* Four shapes since the avatar's F2 checkpoint (`b133c99`): a countdown that collected
   exits resumes with `Aexits` (M6), and a settled race resumes its host with a *program*
   (`Aprogram`, R2-12) -- a closure the snapshot can only name. *)
let json_of_answer (a : answer) : E4d_json.t =
  ...
  | Aexits exits -> Obj [ ("tag", Str "exits"); ("exits", List (List.map json_of_exit exits)) ]
  | Aprogram _ ->
    Obj [ ("tag", Str "program"); ("note", Str "the race settle program, run on the host's own stack") ]
...
let json_of_pending (p : pending) : E4d_json.t =
  Obj
    [ ("token", Int p.token);
      ...
      (* the targets the countdown walk has not visited yet (R2-4), in input order *)
      ("remaining", of_ints p.remaining);
      ("collected", List (List.map json_of_exit p.collected));
      ("failFast", Bool p.fail_fast);
      ( "resumeWith",
        Str
...
let json_of_race (r : race) : E4d_json.t =
  Obj
    [ ("id", Int r.rid);
      ...
      (* the entrants not yet forked: one per iteration of the register loop (R2-11) *)
      ("programsPending", Int (List.length r.programs)) ]
...
(* The service state: the three stores of `src/Effect4/Machine/Stores.lean` and the Layer memo
...
(* A completed Deferred holds the *program* its awaiters run (`Deep_stores.program`, a
   closure) rather than a `completion` value, so the snapshot can only say that one is
   held. *)
let json_of_completion (c : Deep_stores.program option) : E4d_json.t =
  match c with
  | Some _ -> Obj [ ("tag", Str "program"); ("note", Str "completed: a program the awaiters run") ]
  | None -> Null
        ...
        List
          (List.mapi
             (fun i (c : Deep_stores.deferred_cell) ->
               Obj
                 [ ("index", Int i);
                   ("completion", json_of_completion c.Deep_stores.completion);
                   ( "waiters",
                     List
                       (List.map
...
let json_of_event (e : run_event) : E4d_json.t =
  ...
  let fields =
    ...
    (* `57924eb` (R2-11): the entrant *count*; an entrant has no id before its launch, and
       `raceSkipped` is retired with it. *)
    | RaceStarted (r, h, n) -> [ ("race", Int r); ("host", Int h); ("entrants", Int n) ]
...
let event_fiber (e : run_event) : int option =
  ...
  | RaceLaunched _ | RaceSettled _ | CallbackEv _ -> None
