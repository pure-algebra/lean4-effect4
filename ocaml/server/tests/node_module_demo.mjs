...
// tests/ -> server/ -> ocaml/: dune's own output is the default; `W2_BUILD` overrides and
// names the directory holding effect4d.js itself; an explicit argv[2] wins (dune-test.sh
// passes one, a Windows path under WSL interop).
const here = path.dirname(fileURLToPath(import.meta.url));
const buildDefault =
  process.env.W2_BUILD ||
  path.join(here, "..", "..", "_build", "default", "server");
const target = process.argv[2] || path.join(buildDefault, "effect4d.js");
